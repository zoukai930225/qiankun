...
const setChartOptions = () => {
  let xAxis = {
    ...
    axisLabel: {
      ...
      interval: props.option.xAxisData && props.option.xAxisData.length >= 30 ? 0 : 0, //设置 X 轴数据间隔几个显示一个，为0表示都显示
      color: '#888',
      fontSize: 12,
      fontWeight: 400,
      formatter: function (value) {
        ...
        if (value.length > maxLength) {
          ...
          const end = value.slice(-3) // 获取后半部分
          return start + '...' + end //
        }
        return value
      }
  ...
  //    console.log('*',props.option1.xAxisData);
  //     if (props.option1.xAxisData.length > 0) {
  //     console.log('0', param[0]);
  //     console.log('1', param[1]);
  //     console.log('2', param[2]);
  //     console.log('3', param[3]);
  //       return (
  //         '<div style="color: #ffffff;fontSize: 14px;width:184px;"> ' +
  //         `${param[0].axisValue}` +
  //         '<div style="display: flex;flex-direction: row;align-item:cebter;height:30px;margin-bottom:-10px;margin-top:10px;">' +
  //         '<span>' +
  //         `${props.option.tipsName}` +
  //         '</span>' +
  //         '<span style="margin-left:auto;">' +
  //         `${props.option.percent ? param[0].value : numberFormateShow(param[0].value)}${
  //           props.option.percent ? '%' : ''
  //         }` +
  //         '</span>' +
  //         '</div>' +
  //         '<div style="display: flex;flex-direction: row;align-item:cebter;height:30px;margin-bottom:-10px;">' +
  //         '<span>' +
        ...
        if (param && param.seriesName && param.value !== undefined) {
          ...
          switch (param.seriesName) {
            ...
            case props.option2.tipsName:
              ...
              break
            case props.option3.tipsName:
              tipsName = props.option3.tipsName
              value = param.value
              percentSign = '%'
              break
          }
  ...
  if (props.option3.xAxisData.length > 0) {
    series[3] = {
