...
const props = defineProps({
  height: propTypes.oneOfType([Number, String]).def('135px'),
...
const setChartOptions = () => {
  let xAxis = {
    ...
    axisLabel: {
      ...
      interval: props.option.xAxisData && props.option.xAxisData.length >= 15 ? 0 : 0, //设置 X 轴数据间隔几个显示一个，为0表示都显示
      color: '#98999B',
      fontSize: 8,
      fontWeight: 400,
      show: true
  ...
  let tooltip = {
    ...
    formatter: function (param) {
      return (
        ...
        `<div style="width: 6px;height: 6px;background: #FF8400;border-radius:3px;margin-right:4px;"></div>` +
        `<div>` +
        `${param[0].value}${props.option.isPercent ? '%' : ''}` +
        '</div>' +
        '</div>'
      )
      ;('</div>')
