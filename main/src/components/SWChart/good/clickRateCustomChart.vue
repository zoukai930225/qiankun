...
const setChartOptions = () => {
  ...
  let tooltip = {
    ...
    formatter: function (param) {
      return (
        '<div style="width: 100px;height: 46px;background: transparent;border-radius: 4px;display: flex;flex-direction: column;margin:-4px; ">' +
        `<div style="display: flex;flex-direction: row;align-items: center;padding-top: 4px;margin-left:6px;font-size: 14px;color: #FFF;">` +
        '<div>' +
        `${param[0].axisValue}` +
        '</div>' +
        '</div>' +
        `<div style="display: flex;flex-direction: row;align-items: center;padding-top: 0px;margin-left:6px;font-size: 14px;color: #fff;">` +
        `<div style="width: 13px;height: 4px;background:${props.option1.lineColor};border-radius:4px;margin-right:4px;"></div>` +
        `<div>` +
