...
const disabledDate = (time: Date) => {
  ...
  if (props.useNewRule && selectType.value === 4) {
    //当前周不能点击
    return (
      time.getTime() >
      new Date(dayjs(dayjs().startOf('week').subtract(1, 'day')).endOf('week')).getTime()
    )
  }
...
const timeSelectAction = (value) => {
  ...
  selectIndex.value = value.type - 1
...
const arrowLeftClick = () => {
  if (selectType.value == 3) {
    ...
    // 修改时间格式为 2025-05-01
    dateValue.value = dayjs(currentTimeStamp).format('YYYY-MM-DD')
    selectDateRange.value = [dateValue.value, dateValue.value]
    staticsTimeInfo.value = getStaticsTimeDetail(dateValue.value, '')
    sendDateSelect()
  } else if (selectType.value == 4) {
    ...
    if (selectDateRange.value && selectDateRange.value[0] && selectDateRange.value[1]) {
      ...
      const beginDate = dayjs(beginTimeStamp).format('YYYY-MM-DD')
...
const arrowRightClick = () => {
  if (selectType.value == 3) {
    ...
    currentTimeStamp = currentTimeStamp + 24 * 60 * 60 * 1000
    //超过最大允许时间，返回
    if (MaxTime.value && currentTimeStamp > DateUtil.dateToTimestamp(MaxTime.value)) {
      message.error('超过最大允许时间')
      return
    }
    dateValue.value = dayjs(currentTimeStamp).format('YYYY-MM-DD')
    staticsTimeInfo.value = getStaticsTimeDetail(dateValue.value, '')
    ...
    if (selectDateRange.value && selectDateRange.value[0] && selectDateRange.value[1]) {
      ...
      endTimeStamp = endTimeStamp + weekTimeStamp
      //超过最大允许时间，返回
      if (MaxTime.value && endTimeStamp > DateUtil.dateToTimestamp(MaxTime.value)) {
        message.error('超过最大允许时间')
        return
      }
      const beginDate = dayjs(beginTimeStamp).format('YYYY-MM-DD')
...
if (props.defaultDateByQuery) {
  ...
  // staticsTimeInfo.value = getStaticsTimeDetail(lastDay.split(' ')[0], '')
  // staticsTimeInfo.value = MaxTime.value
  // dateValue.value = lastDay.split(' ')[0]
}
