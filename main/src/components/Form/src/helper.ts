...
import { customComponentMap } from './componentMap'
 ...
 * @description 生成对应的formModel
 */
export const initSearchFormModel = (schema: FormSchema[], formModel: Recordable) => {
  const model: Recordable = { ...formModel }
  schema.map((v) => {
    const field = v.field as any
    // field 为空则不处理
    if (!field) {
      return
    }
    ...
    if (v.hidden) {
      ...
      const hasField = Reflect.has(model, field)
      // 如果先前已经有值存在，则不进行重新赋值，而是采用现有的值
      model[field] = hasField ? model[field] : v.value !== void 0 ? v.value : ''
    }
  })
  return model
}
 ...
 * @description 生成对应的formModel
 */
export const initSchema = (schema: FormSchema[]) => {
  const model: Recordable = {}
  schema.map((v) => {
    // 如果是hidden，就删除对应的值
    if (v.hidden) {
      delete model[v.field]
    } else if (v.component && !customComponentMap.includes(v.component as string)) {
      // const hasField = Reflect.has(model, v.field)
      model[v.field] = v.value !== void 0 ? v.value : ''
    }
  })
