<template>
  <ElDialog v-if="isModal" v-model="showSearch" :show-close="false" title="菜单搜索">
    <el-select filterable :reserve-keyword="false" remote placeholder="请输入菜单内容" :remote-method="remoteMethod"
      style="width: 100%" @change="handleChange">
      <el-option v-for="item in options" :key="item.value" :label="item.label" :value="item.value" />
    </el-select>
  </ElDialog>
  <div v-else class="custom-hover">
    <!-- <Icon icon="ep:search" @click.stop="onFocus" /> -->
    <el-select ref="selectRef" filterable :reserve-keyword="false" remote placeholder="请输入菜单名称搜索"
      :remote-method="remoteMethod" class="overflow-hidden transition-all-600"
      :class="showTopSearch ? '!w-220px ml2' : '!w-0'" @change="handleChange">
      <el-option v-for="item in options" :key="item.value" :label="item.label" :value="item.value" />
    </el-select>
  </div>
</template>
...
const router = useRouter(), selectRef = ref(); // 路由对象
...
const onFocus = () => {
  ...
  showTopSearch.value && selectRef?.value?.focus();
};
