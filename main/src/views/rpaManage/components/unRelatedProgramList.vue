...
/** 打开弹窗 */
const open = async (paramName: string, isCommonParam: string, id: string) => {
...
const handleSelectionChange = (val: any[]) => {
  ...
  // console.log('多选', multipleSelection.value)
}
/** 分页 */
...
const getList = async () => {
  ...
  try {
    queryParams.value.runEnvironment = queryParams.value.runEnvironment
      ...
      : ''
    const data = await rpaApi.getUnbindProgramListByParamId(queryParams.value).catch(() => {
      loading.value = false
    })
...
const submitForm = async () => {
  console.log('表单数据', multipleSelection.value)
  // 提交请求
  formLoading.value = true
  try {
    if (!multipleSelection.value || multipleSelection.value.length === 0) {
      message.warning('请选择要关联的应用！')
      return
    }
    ...
    await rpaApi.configBindProgram(data)
