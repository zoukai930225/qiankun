...
const emit = defineEmits(['update:modelValue', 'update:operatorName', 'update:userId','update:directorAvatar'])
...
function changeModule(value: any) {
  ...
  emit(
    'update:directorAvatar',
    OperatorRosterList.value.find((el) => el.userId === value[value.length - 1])?.avatarOrigin
  )
}
