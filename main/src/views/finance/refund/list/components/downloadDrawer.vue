  ...
  <SWDrawer v-model="drawerVisible" title="结果下载" @close="reset">
    <div class="content">
      ...
      <el-table :header-cell-style="appStore.headerCellStyle" v-loading="loading" :data="list"
        ...
        <el-table-column label="状态" prop="exportStatus" min-width="150">
          <template v-slot="{ row }">
            <div class="row-center">
              <div class="status-tag" :class="'status-tag-' + row.exportStatus">{{ Number(row.exportStatus) === 1 ?
                '生成中' : Number(row.exportStatus) === 2 ? '已生成' : '生成失败' }}
              </div>
            </div>
          </template>
        </el-table-column>
        <el-table-column label="操作" width="80">
          <template #default="{ row }">
            <el-button link type="primary" @click="handleDownload(row)"
              v-if="Number(row.exportStatus) === 2">下载</el-button :loading="row.loading">
            <span v-else>-</span>
          </template>
        </el-table-column>
        <template #empty>
...
import * as finaceApi from "@/api/finance/zto";
...
const getList = async (e: any = { showLoading: true }) => {
  ...
  try {
    ...
    const data = await finaceApi.getZtoExportPageList(queryParams.value)
...
const handleDownload = debounce(async (row: any) => {
  try {
    row.loading = true
    await downloadByUrl2(row.fileOssUrl, row.fileName)
  } catch (e) { } finally {
    row.loading = false
  }
}, 1000)
