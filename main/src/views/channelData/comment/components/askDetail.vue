<template>
  <el-table :header-cell-style="appStore.headerCellStyle" v-loading="loading" :data="dataList" border
    ...
    <el-table-column align="center" prop="tody" :min-width="200">
      <template #header>
        <span class="row-center" style="justify-content: center;">最新一次采集（{{ todoyDate }}）
        </span>
      </template>
      <template v-slot="{ row }">
        <Card :row="row.tody" @toggleExpand="toggleExpand" />
      </template>
    </el-table-column>
    <el-table-column align="center" prop="yester" :min-width="200">
      ...
      <template v-slot="{ row }">
        <Card :row="row.yester" @toggleExpand="toggleExpand" />
      </template>
    </el-table-column>
    <template #empty>
...
import Card from './askDetailCard.vue'
...
const onSearch = () => {
  loading.value = true;
  request.get({ url: `/api/odsTaoProductDetailComments/getQuestionDetail`, params: { dataDate: commentForm.searchData.value.dataDate, goodsId: CommentDetails.dataValue.value.goodsId, store: CommentDetails.dataValue.value.store } }).then((res: any) => {
    const data = [] as any[]
    if (res?.todoyDate && Array.isArray(res?.todayQuestionVoList)) {
      todoyDate.value = res.todoyDate;
      yesterDate.value = res.yesterDate;
      res.todayQuestionVoList.map((item, index) => {
        data.push(
          {
            tody: item,
            yester: res.yestertodayQuestionVoList[index],
            color: item.color || res.yestertodayQuestionVoList[index].color || '',
            expand: false
          }
        )
      })
