    ...
    <el-drawer custom-class="editDrawer" v-model="drawerVisible" :title="dialogTitle" size="968">
      <div class="drawerContent">
        <div class="drawerContent-title">
          ...
          <div>品类：{{ formData.category }}</div>
        </div>
        <el-form class="dialogForm" :model="formData" label-width="105px" ref="formRef">
...
const submitForm = async () => {
  ...
  try {
    // 清洗内容
    formData.value.yesterdayReview = cleanContent(formData.value.yesterdayReview)
    formData.value.todayOperation = cleanContent(formData.value.todayOperation)
...
const cleanContent = (content: string): string => {
  ...
  if (trimmed === '<p><br></p>' || trimmed === '<div><br></div>' || trimmed === '') {
    return ''
  }
  return trimmed
}
