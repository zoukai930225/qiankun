<template>
  ...
  <div class="mainContainer">
    ...
    <div>
      <ChannelHeader :date="dynamicTitle" :store="'PDD'" />
...
import CommonTable from '../components/pddCommonTable.vue'
...
const getDefaultDate = async () => {
  const data = await ChannelDataApi.getPddDefaultDate({ ...queryParams, type: '1' }).catch(() => {})
...
const getList = async () => {
  ...
  try {
    ...
    const data = await ChannelDataApi.getPddPlatformList(queryParams)
    list.value = data || []
