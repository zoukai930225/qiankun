<template>
  <el-table class="SWCommonTable" v-horizontal-scroll="'always'" v-loading="loading" show-overflow-tooltip
    ref="tableRef" :border="border" :header-cell-style="appStore.headerCellStyle" :stripe="stripe" :data="tableData"
    :max-height="maxHeight" :height="height" :row-key="rowKey" :span-method="spanMethod"
    @selection-change="(val) => emit('selection-change', val)">
    <el-table-column v-for="item in options" :key="item.prop" :label="item.label" :fixed="item.fixed"
      :align="item.align" :width="item.width" :type="item.type">
    ...
    <template #empty>
      <img :style="{ width: emptySize, height: emptySize }" ref="emptyImg" class="mt16px"
        src="@/assets/imgs/empty/empty1.png" alt="" />
