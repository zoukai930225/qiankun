<template>
  <div v-loading="exportLoading" element-loading-text="正在导出...">
    <stepBox />
    <div class="mainWrap">
      <!-- 搜索工作栏 -->
            ...
            <template #default="scope">
              <div class="action-wrap">
                <el-button
                  ...
                  @click="openForm('view', scope.row.appraiseeId, scope.row.status)"
                  >校准意见</el-button
                >
                <el-button class="action-edit" v-if="false" link @click="reportSend(scope.row)"
                  >报告发送</el-button
                >
                ...
                <el-button
                  ...
                  :loading="pdfLoading && donwloadId === scope.row.appraiseeId"
                  @click="reportDownload(scope.row)"
                  >报告下载</el-button
                >
              </div>
            </template>
  ...
  <OTRPdfDownload
    class="OTRPdfDownload"
    ref="OTRPdfDownloadRef"
...
import NineGrid from '@/views/OTR/InventoryCalibration/components/nineGrid.vue'
import Axis from '@/views/OTR/InventoryCalibration/components/axis.vue'
import OTRPdfDownload from '@/views/OTR/pdf/download.vue'
import AddProdForm from '@/views/OTR/InventoryCalibration/components/addProdForm.vue'
...
import tooltipItem from '@/views/OTR/InventoryCalibration/components/tooltipItem.vue'
import DraggableTooltip from '@/views/OTR/InventoryCalibration/components/draggableTooltip.vue'
import nineGridDialog from '@/views/OTR/InventoryCalibration/components/nineGridDialog.vue'
...
const getTableHeader = async () => {
  ...
  if (Object.keys(data).length > 0) {
    ...
    console.log('dimension1Data', dimension1Data.value[firstKey])
    ...
    console.log('000', dimension1DataName)
    console.log('k', lastTableDefine.value)
    // 遍历数组，找到匹配的项并赋值给dimensionAve ---维度1均分
    lastTableDefine.value.forEach((item) => {
    ...
    console.log('aa', lastTableDefine.value)

    // 使用filter方法删除匹配的项
    lastTableDefine.value = lastTableDefine.value.filter((item) => {
...
const reportView = (row: any) => {
  // router.push({
  //   path: '/otr/pdf',
  //   query: { inventoryId: row.inventoryId, appraiseeId: row.appraiseeId }
  // })
...
// 报告发送
const reportSend = async (row: any) => {
  // 状态的二次确认
  await message.confirm('确定发送报告?', '提示')
  await InventoryCalibrationApi.otrPdfFeiShuSend({
...
const OTRPdfDownloadRef = ref()

const pdfLoading = ref(false)
const donwloadId = ref('')
// 报告下载
const reportDownload = (row: any) => {
  pdfLoading.value = true
  donwloadId.value = row.appraiseeId
  OTRPdfDownloadRef.value?.downloadPdf({
    inventoryId: row.inventoryId,
    appraiseeId: row.appraiseeId,
    title: row.name
  })
...
const handleExport = async () => {
  try {
    ...
    const res = await InventoryCalibrationApi.exportCalibrationResult(queryParams.value)
    if (res) {
      download.excel(res, `人才盘点结果.xls`)
...
const getTableMaxHeight = () => {
  ...
  console.log('filterWrapHeigh1111t', filterWrapHeight, window.innerHeight)
