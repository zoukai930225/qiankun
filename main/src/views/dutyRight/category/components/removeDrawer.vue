<template>
  <scheme-details ref="detailsRef" name="CategoryRemove" :width="1000">
    <template #card>
      <div>
        <div class="row-center" style="align-items: flex-start;">
          <el-form :model="dataValue" label-width="80" :rules="rules" ref="formRef" label-position="top"
            style="width: 430px;">
            <el-form-item prop="completeCategoryNames" label="类目">
              <categorySelect v-model:channel="channel" v-model="dataValue.completeCategoryNames"
                @channelChange="handleChannelChange" @change="getDirectors" ref="categorySelectRef"
            ...
            <el-form-item prop="stores" label="店铺">
              <StoreCascader :filter-channel="[channel]" ref="storeCascaderRef" v-model="dataValue.stores"
                @change="getDirectors" :otherProps="{ disabled: !channel }" :cascaderProps="{ multiple: true }" />
            </el-form-item>
          </el-form>
          <div style="width: calc(100% - 450px);" class="ml10px">
        ...
        <personnalPanel @change="handleCheckChange" ref="personnalPanelRef" :departmentAllList="departMentList"
          :loading="departLoading" :top-height="419" :repeat="7" opType="delete" />
      </div>
...
<script lang="tsx">
export default { name: 'CategoryRemove' };
</script>
...
const detailsRef = ref<any>(), formRef = ref(), dataValue = ref<DataItem>({ ...new DataItem() }), bukValue = ref<DataItem>({ ...new DataItem() }), viewType = ref<string>('edit'), loading = ref<boolean>(false), personnalPanelRef = ref<any>(), storeCascaderRef = ref<any>(), departMentList = ref<any[]>([]), departLoading = ref(false), storeTagList = ref<any[]>([]), categorySelectRef = ref<any>();
...
const getDirectors = () => {
  departMentList.value = []
  if (dataValue.value.completeCategoryNames.length === 0 || dataValue.value.stores.length === 0) {
    personnalPanelRef.value?.setCheckIds([])
    return
  }
...
const submit = debounce(async () => {
  ...
  request.post({ url: `/api/opDutyAuthorityManage/removeBatchCategoryDutyAuthority`, data: params }).then(() => {
...
provide('CategoryRemoveDetails', { dataValue, bukValue, viewType, title });
