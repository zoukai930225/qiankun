<template>
  <ElScrollbar>
    <div class="bg">
      ...
      <div class="bg-title">
        <img class="bg-title-left" src="@/assets/imgs/dataV/sale/weekly_rank_after.png" />
        ...
        >
          ...
          >
            <div class="item-group-value">{{
              index < afterWeekTopRankGropuList().length
                ? afterWeekTopRankGropuList()[rankIndex(index)]
                : ''
            }}</div>
          </div>
        </div>
...
const afterWeekTopRankGropuList = () => {
  const list = []
  props.topList &&
    props.topList.forEach((item, index) => {
