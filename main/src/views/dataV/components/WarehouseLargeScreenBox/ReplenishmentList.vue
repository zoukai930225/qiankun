<!-- 补货排行榜 毛天 2025年3月11日11:15:52 -->
<template>
  <div class="box">
    <div class="top">
      <!-- <div class="top-iocn"></div>
      补货排行榜 -->
      <img src="@/assets/imgs/dataV/bhListTitle.png" alt="" />
    </div>
    <div class="topName">
    ...
    <div class="guding">
      <div class="bottom-top">
        ...
        <div class="bottom-top-two">补货员</div>
        <div class="bottom-top-three">补货次数</div>
        <div>货品种类数</div>
      </div>
    </div>
    <div class="bottom" ref="rankScrollRef">
      <div class="scroll-content">
        <div class="list-item" v-for="(item, index) in list" :key="index">
          ...
          <div class="list-item-two">{{ item.operator }}</div>
          <div class="list-item-three">{{ item.replenishmentCount }}</div>
          <div>{{ item.goodsTypeNumber }}</div>
        </div>
      </div>
    </div>
  </div>
...
const init = async () => {
  ...
  const res = await warehouseLargeScreen.replenishmentPersonnelRank(params)
  list.value = res
  if (list.value.length >= 5) {
    list.value = [...list.value, {}]
  }
  No1.value = res[0].operator || ''
  No2.value = res[1].operator || ''
  No3.value = res[2].operator || ''
}
// 每隔10分钟请求一次接口
...
.box {
  ...
  .bottom-top {
    ...
    .bottom-top-one {
      ...
      margin-right: 4.46875vw;
    }
    .bottom-top-two {
      width: 7vw;
      margin-right: 1.66875vw;
    }
    .bottom-top-three {
      width: 7vw;
