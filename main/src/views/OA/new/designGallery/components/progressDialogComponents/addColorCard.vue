  ...
  <el-dialog title="应用范围" v-model="dialogVisible" :show-close="false" style="
    ...
    <div class="container">
      <el-form :model="form" label-width="53px" :rules="rules" ref="formRef">
        ...
        <el-form-item label="色卡" prop="attachmentUrl">
          <SWUploadFile ref="swUploadFileRef" v-model:modelValue="form.attachmentUrl" :width="473" />
        </el-form-item>
      </el-form>
    </div>
...
import { addColorCard } from '@/api/oa/new/designGallery/index'
...
const open = (item) => {
  form.value.remark = ''
  form.value.attachmentUrl = undefined
...
const rules = {
  ...
  attachmentUrl: [{ required: true, message: '请上传色卡', trigger: 'blur' }]
}
...
const submitForm = async () => {
  ...
  if (form.value.attachmentUrl && form.value.attachmentUrl.length > 0) {
    ...
    form.value.attachmentUrl.forEach((element) => {
  ...
  //二次确认提交逻辑
  message
    .confirm('请仔细核对色卡信息，确认提交？', '系统提示')
    .then(async () => {
      // 提交逻辑
      console.log('提交表单', form.value)
      const params = {
        npDesignPictureId: currentDetail.value.npDesignPictureId,
        npSupplierAuditStatusId: currentDetail.value.id,
