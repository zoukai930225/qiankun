...
watch(
  () => [newProductInfo.planName, list.value],
  ([planName, tableData]) => {
    if (planName && tableData.length) {
      const target: any = tableData.find((row: any) => {
        return row.planName === planName
      })
      if (!target) return
      currentSelectId.value = target.id
      form.value.enterprisePlanId = target.id
...
onActivated(async () => {
  form.value.planName = newProductInfo.planName
  handleQuery()
})
