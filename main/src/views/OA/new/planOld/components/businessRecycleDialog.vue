<template>
  <div>
    <SWDialog v-if="visible" v-model="visible" :title="dialogTitle" :width="width" :showTitleLine="true"
      :show-bottom-line="true" :title-warn-icon="dialog_title_warn" title-warn="数据回收后流程将暂停，数据不再显示">
      <div class="dialog-content" style="
        ...
        ">
        <el-form ref="ruleFormRef" :model="form" label-width="auto" :rules="rules" @click="formBgClick">
          <el-form-item label="回收原因" prop="recycleReason">
            <s-text-area v-model="form.recycleReason" placeholder="请上输入回收原因" style="width: 480px; color: #333333"
              :rows="5" />
          </el-form-item>
          <el-form-item label="附件" prop="recycleAttachmentUrl">
            <SWUploadFile style="width: 258px" ref="fileDataUploadFileRef"
              v-model:modelValue="form.recycleAttachmentUrl" :width="258" :hideFileSizeMsg="true" />
          </el-form-item>
        </el-form>
      </div>
      <template #footer>
...
const confirmDialog = async () => {
  ruleFormRef.value.validate(async (valid) => {
    if (valid) {
      ...
      try {
        // 二次确认删除逻辑
        await message.confirm('数据回收后流程将暂停，数据不再显示', '系统提示')
