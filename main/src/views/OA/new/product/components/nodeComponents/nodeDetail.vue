<template>
  <div class="popver-main">
    ...
    <div :class="[
      ...
      tab.status === '0' ? 'status-gray' : '',
      (Number(tab.status) || 0) === 1 && tab.finishStatus && tab.finishStatus === '延期'
        ? 'status-red'
        : ''
    ]">
      <img style="width: 18px; height: 18px" v-if="(Number(tab.status) || 0) !== 1"
        :src="getDotImg(Number(tab.status) || 0, tab.nodeCode, nodeList)" alt="" />
      <img class="endStatus" v-else-if="
        (Number(tab.status) || 0) === 1 && tab.finishStatus && tab.finishStatus === '正常'
      " src="@/assets/imgs/oa/node/node_endStatus_blue.png" />
      <img class="endStatus" v-else-if="
        (Number(tab.status) || 0) === 1 && tab.finishStatus && tab.finishStatus === '延期'
      " src="@/assets/imgs/oa/node/node_endStatus_red.png" />
      <img class="endStatus" v-else-if="(Number(tab.status) || 0) === 1"
        src="@/assets/imgs/oa/node/node_endStatus_blue.png" />
      <span>{{
        (Number(tab.status) || 0) === 1
          ? tab.finishStatus === '正常'
            ? '正常'
            : tab.finishStatus === '延期'
              ? '延期'
    ...
    <div v-if="Number(tab.isExamine || 0) === 1" style="
        overflow: auto;
        overflow-x: hidden;
      ...
      <div style="width: 200px; padding-bottom: 4px; padding-left: 8px; padding-right: 8px">
        <div class="progress-item__value textWrap" style="padding-top: 8px; padding-left: 6px">
          <div v-for="(value, index1) in parseExamineInfoRemarks(tab)" :key="index1"
            style="padding-top: 2px; padding-bottom: 2px">
            <div class="row-center">
              <div style="font-weight: 500; font-size: 12px; color: #333333; padding-right: 5px;max-width: 124px;">{{
                getDictLabel(
              ...
              <div class="row-center" style="height: 23px; background: #e6ecff; border-radius: 13px">
                <img style="
                    width: 15px;
                    height: 15px;
                    border-radius: 8px;
                ...
                <div v-if="value.examineNodePersonName" style="font-size: 12px; color: #333333; padding-right: 7px">{{
                  value.examineNodePersonName }}</div>
              </div>
            </div>
            <div v-if="value.examineNodeRemark"
              style="font-size: 12px; color: #666666; padding-top: 8px; padding-bottom: 4px">{{ value.examineNodeRemark
              }}</div>
            <div v-if="value.examineNodeFile && parsedDocUrls(value.examineNodeFile).length > 0" class="row-center"
              style="
                margin-bottom: 5px;
                padding-top: 3px;
                padding-right: 6px;
                ...
                border-radius: 4px;
              ">
              <img src="@/assets/imgs/oa/node/node_link.png" alt=""
    ...
    " style="
      ...
      <div style="width: 200px; padding-bottom: 4px; padding-left: 8px; padding-right: 8px">
        <div style="padding-top: 8px; padding-left: 6px">
          <div class="row-center">
            <div v-if="getNodeSubmitter(tab)" class="row-center"
              ...
              <div v-if="getNodeSubmitter(tab).name" style="font-size: 12px; color: #333333; padding-right: 7px">{{
                getNodeSubmitter(tab).name }}</div>
            </div>
          </div>
          <div v-if="tab.remarks" style="font-size: 12px; color: #666666; padding-top: 8px; padding-bottom: 4px">{{
            tab.remarks }}</div>
          <div class="row-center" style="
              ...
              background: #e9eefa;
              border-radius: 4px;
            " v-for="(url, index) in parseExamineInfoFiles(tab)" :key="index">
            <img src="@/assets/imgs/oa/node/node_link.png" alt=""
              style="margin-left: 6px; margin-right: 5px; width: 10px; height: 11px" />
