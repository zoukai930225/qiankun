...
<template>
  <div>
    <SWDrawer
      ...
      v-model="isVisible"
      :width="936"
      :title="isAduit ? '详情' : currentId ? '编辑' : '新增'"
      :showBeforClose="true"
    ...
    >
          ...
          <ContentWrap>
            <div class="form-wapper">
              ...
              >
                  ...
                  <div style="flex-wrap: wrap; flex: 1" class="flex-row">
                    <div v-for="(field, index) in newFields()" :key="index">
                      ...
                      >
                        ...
                        <!-- 企划时间 -->
                        <div
                          v-if="field.code === 'planningTime'"
                          :style="{ width: `${itemWidthDouble}px` }"
                          class="planningTimeDatePicker"
                        >
                          <el-date-picker
                            :style="{ width: `${itemWidthDouble - 10}px` }"
                            v-model="formInline[field.code]"
                            type="daterange"
                            range-separator="-"
                        ...
                        <div v-if="field.code === 'operateManageId'">
                          <SWSelectPeople
                            ...
                            :isEdit="!isAduit"
                            :multiple="true"
                            v-model:modelValue="formInline.operateManageId"
                          />
                        </div>

                        <!-- 计划上架月份-->
                        <el-date-picker
                          v-if="field.code === 'planListDate' && !isAduit"
                          v-model="formInline[field.code]"
                          type="date"
                          :style="{ width: `${inputWidth}px` }"
                        ...
                        <el-input
                          ...
                          v-model="(formInline as any).planListDate"
                          disabled
                          :style="{ width: `${inputWidth}px` }"
                        />

                        <s-text-area
                          v-if="field.code === 'remark'"
                          v-model="formInline[field.code]"
                          :placeholder="field.placeholder"
                          style="width: 740px"
                        ...
                        <el-radio-group
                          v-if="field.code === 'isCreateGoodsNo'"
                          v-model="formInline.isCreateGoodsNo"
                          :disabled="isAduit"
                        >
                        ...
                        <component
                          ...
                          v-model="formInline[field.code]"
                          :placeholder="componentPlaceholder(field)"
                ...
                >
                  <el-input
                    v-model="formInline['competitionLink']"
                    placeholder="请输入竞品链接"
                    style="width: 770px"
                    :disabled="isAduit"
                  >
                    <template #append>
                      <div
                        ...
                        @click="handleCompetitionLinkClick(formInline['competitionLink'])"
                        >跳转</div
                      >
                    </template>
                  </el-input>
                ...
                >
                  <s-text-area
                    v-model="formInline.productRisk"
                    placeholder="请输入风险点"
                    style="width: 770px"
                    :autosize="{ minRows: 3, maxRows: 3 }"
                    :disabled="isAduit"
                ...
                <div style="display: flex; flex-wrap: wrap">
                  ...
                  <el-form-item
                    ...
                    label-width="78px"
                    :required="true"
                    prop="productDiagramReference"
                    :rules="[{ required: true, message: '产品图参考不能为空', trigger: 'change' }]"
                    :style="{ width: `${itemWidth}px` }"
                  >
                    <uploadFile
                      :style="{ width: inputWidth }"
                      ref="productDiagramReferenceUploadFileRef"
                      v-model:modelValue="formInline.productDiagramReference"
                      :width="inputWidth"
                      :disabled="isAduit"
                  ...
                  >
                    <uploadFile
                      :style="{ width: inputWidth }"
                      ref="fabricPictureUploadFileRef"
                      v-model:modelValue="formInline.fabricPicture"
                      :width="inputWidth"
                      :disabled="isAduit"
                  ...
                  >
                    <uploadFile
                      :style="{ width: inputWidth }"
                      ref="productPriceUploadFileRef"
                      v-model:modelValue="formInline.productPrice"
                      :width="inputWidth"
                      :disabled="isAduit"
                  ...
                  >
                    <uploadFile
                      :style="{ width: inputWidth }"
                      ref="marketAnalysisUploadFileRef"
                      v-model:modelValue="formInline.marketAnalysis"
                      :width="inputWidth"
                      :disabled="isAduit"
                  ...
                  >
                    <uploadFile
                      :style="{ width: inputWidth }"
                      ref="demandAnalysisUploadFileRef"
                      v-model:modelValue="formInline.demandAnalysis"
                      :width="inputWidth"
                      :disabled="isAduit"
          ...
          <ContentWrap>
            <div class="form-wapper">
              ...
              >
                ...
                <el-table-column
                  v-if="addType === 'newGoods'"
                  label="包装图片"
                  prop="unRoutinePackagePicture"
                >
                  <template #default="scope">
                    <el-image
                ...
                >
                  <template #default="scope">
                    <el-image
                      v-if="scope.row.packageProgressPicture"
                      style="width: 30px; height: 30px"
                      :src="scope.row.packageProgressPicture"
                      :preview-teleported="true"
                      :preview-src-list="[scope.row.packageProgressPicture]"
                      fit="cover"
                    />
                    <div v-else>--</div>
                  </template>
                </el-table-column>
                <el-table-column width="120" fixed="right" align="right" v-if="!isAduit">
                  <template #default="scope">
                    <el-button link type="primary" @click="handleEdit(scope.row)"> 编辑 </el-button>
                    <el-button link type="danger" @click="handleDelItem(scope.row)">
                      删除
                    </el-button>
                  </template>
                </el-table-column>
      ...
      <template #footer>
        <div class="btn-wapper" v-if="!isAduit">
          <el-button @click="reset" :disabled="saveLoading">取消</el-button>
          <el-button
            class="templeSaveBtn"
            v-if="!currentId"
            @click="tempSave"
            ...
            >暂存</el-button
          >
          <el-button type="primary" @click="save(false)" :disabled="saveLoading">保存</el-button>
        </div>
      </template>
    </SWDrawer>
    ...
    <SWDialog
      ...
      :showBottomLine="true"
    >
      <div style="width: 770px; overflow-x: hidden; margin-top: 15px;height: 75vh;" @click="bgClick2">
        <!-- <ElScrollbar class="dialog-content" style="height: 75vh"> -->
          <el-form
            :inline="true"
            :model="addForm"
            class="demo-form-inline p6px"
            :rules="itemRules"
            ref="addFormRef"
          ...
          >
            ...
            >
              <div class="row">
                ...
                <div v-if="isAddCatrgory" class="row-center" style="margin-left: 76px">
                  <el-input-number
                    v-if="addForm.productType === '0'"
                    :min="1"
                    v-model="addForm.designNumber"
                  />
                  <el-input-number
                    v-if="addForm.productType === '1'"
                    :min="1"
                    v-model="addForm.marketNumber"
                  />
                </div>
              </div>
            </el-form-item>
            ...
            <el-form-item label="卖点：">
              <Editor
                ...
                height="200px"
                :hideFullScreen="true"
                v-model:modelValue="addForm.salePoint"
              />
            </el-form-item>
            ...
            <div class="row" style="width: 730px">
              ...
              >
                <uploadFile
                  :style="{ width: inputWidth }"
                  ref="unRoutinePackagePictureUploadFileRef"
                  v-model:modelValue="addForm.unRoutinePackagePicture"
                  :width="inputWidth"
                  :disabled="isAduit"
              ...
              >
                <uploadFile
                  :style="{ width: inputWidth }"
                  ref="packageProgressPictureUploadFileRef"
                  v-model:modelValue="addForm.packageProgressPicture"
                  :width="inputWidth"
                  :disabled="isAduit"
      ...
      <template #footer>
        <div class="dialog-footer" style="margin-top: 15px; margin-right: 15px">
          ...
          <el-button v-if="isAddCatrgory" type="primary" @click="continueSaveItem"
            >继续添加</el-button
          >
...
const itemRules = computed(() => {
  return {
    ...
    packageMethod: [
      { required: props.addType === 'newGoods', message: '请选择类型', trigger: 'blur' }
    ],
    unRoutinePackagePicture: [{ required: true, message: '请上传包装图片', trigger: 'change' }],
    packageProgressPicture: [{ required: true, message: '请上传包装流程图', trigger: 'change' }]
  }
})
...
const parseChineseMonth = (chineseMonthStr) => {
  if (chineseMonthStr && chineseMonthStr.endsWith('日')) {
    ...
    if (match) {
      ...
      const day = parseInt(match[3], 10) // 月份是从0开始的
...
const open = async (id?, isView = false, copy = false) => {
  ...
  if (id) {
    ...
    try {
      ...
      if (isCopy.value) {
        currentId.value = ''
        formInline.value.id = currentId.value
        formInline.value.planName = ''
      }
...
const getCategoryName = (categoryName: string) => {
  addForm.value.completeCategoryName = categoryName
}
