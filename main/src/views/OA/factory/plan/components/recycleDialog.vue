...
const { onSearch } = inject('FactoryplanForm') as FromProvideType
  ...
  ruleFormRef?.value?.validate(async (valid) => {
    if (valid) {
      try {
        ...
        await request.post({
          url: `/workflow-api/api/workflow/business/factorySecondary/recycledProduct/supplier/recycle`,
          data
        })
...
const openDialog = (item: any, type: string) => {
  recycleDialogRef?.value?.openDialog(
    () => {
      dataValue.value = { ...new DataValue(), fsTaskId: item.fsTaskId, supplierId: item.supplierId }
