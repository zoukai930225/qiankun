  ...
  <div ref="cardRef" :style="{ height: `${height}px`, marginTop: '5px' }">
    <el-scrollbar ref="scrollbarRef">
      <div class="card">
        ...
        <div class="inner">
          <el-form :model="formValue" ref="infoRef" label-width="120px" :rules="formRules"
            ...
            <el-form-item label="含税裸货价(元)" prop="productCostIncludingTax">
              <!-- -->
              <el-input-number v-model="(formValue.productCostIncludingTax as any)" :precision="3" :controls="false"  @change="sum"
                :max="999999999.999" style="width: 100%;">
              </el-input-number>
            </el-form-item>
            <el-form-item label="包装人工费(元)" prop="packagingManpower">
              <!-- @change="sum" -->
              <!--   <template #suffix>元</template> -->
              <el-input-number v-model="(formValue.packagingManpower as any)" :precision="3" :controls="false"  @change="sum"
                :max="999999999.999" style="width: 100%;">
              </el-input-number>
            </el-form-item>
            <el-form-item label="成品价(元)" prop="finishedProductCost">
              <el-input-number v-model="(formValue.finishedProductCost as any)" :precision="3" :controls="false"
                :max="999999999.999" style="width: 100%;">
              </el-input-number>
            </el-form-item>
          ...
          <el-form :model="formValue" ref="formsRef" label-width="100px" :inline="true" :rules="formsRules">
            ...
            <el-form-item label="报价单" prop="quotationSheetFile" style="margin-bottom: 0px !important;width: 97.5%">
              <transfer :upload-data="{ code: 'FS_SUPPLIER_PLAN' }" :width="473" v-model="formValue.quotationSheetFile"
                :limit="20" :tip="'上传大小不超过20M的文件'" accept-type="all" />
            </el-form-item>
          </el-form>
        </div>
      </div>
        ...
        <vxe-table :header-cell-style="{ color: '#333', fontweight: 500, backgroundColor: '#F2F6FC' }"
          ...
          <vxe-column field="productCostIncludingTax" title="含税裸货价">
            <template #default="scope">
              ...
              <vxe-input v-model.trim="scope.row.productCostIncludingTax">
                <template #suffix>元</template>
              </vxe-input>
            </template>
          </vxe-column>
          <vxe-column field="packagingManpower" title="包装人工费">
            <template #default="scope">
              <!-- @change="(value: any) => numChange(scope, value)" -->
              <vxe-input v-model.trim="scope.row.packagingManpower">
                <template #suffix>元</template>
              </vxe-input>
            </template>
          </vxe-column>
...
const formRules = computed(() => ({
  productCostIncludingTax: [{ required: true, message: '请输入含税裸货价', trigger: ['blur', 'change'] }],
...
const sum = () => {
  formValue.value = { ...formValue.value, finishedProductCost: Number((formValue.value.finishedProductCost || 0)).toFixed(3) as any }
};
  ...
  request.get({ url: `/workflow-api/api/workflow/business/factorySecondary/supplierInfo/queryWdtSpec`, params: { fsTaskId: props.row.id } }).then((res: any) => {
    if (formValue.value?.colorSizeGroups?.length) {
      ...
      tableData.value = (res || []).map((rs: any) => ({
        ...
        productCostIncludingTax: String(formValue.value.productCostIncludingTax || ''),
        finishedProductCost: Number((formValue.value.finishedProductCost || 0)).toFixed(3),
      }));
    }
...
const checkValue = () => {
  let backList: Array<boolean> = [
    dataDetection(
      ...
      ``,
      (item: any) => {
        !isNumber(item.productCostIncludingTax, 3) && ElMessage({
          type: 'warning',
          message: `${item.sku}的裸货价(含税)只能是三位数字!`
        });
    ...
    dataDetection(
      ...
      ``,
      (item: any) => {
        !isNumber(item.packagingManpower, 3) && ElMessage({
          type: 'warning',
          message: `${item.sku}包装人工费只能是三位数字!`
        });
    ...
    dataDetection(
      ...
      ``,
      (item: any) => {
        !isNumber(item.finishedProductCost, 3) && ElMessage({
          type: 'warning',
          message: `${item.sku}的成品价只能是三位数字!`
        });
...
const numChange = (scope: any, value: any) => {
  ...
  tableData.value.splice(index, 1, {
    ...tar,
    finishedProductCost: Number(Number(tar?.finishedProductCost || 0)).toFixed(3) as any
  })
};
...
const addGoods = (list: any) => {
  tableData.value = [...tableData.value, ...list.map((rs: any) => ({
    ...
    productCostIncludingTax: formValue.value.productCostIncludingTax,
    finishedProductCost: Number((formValue.value.finishedProductCost || 0)).toFixed(3),
  }))]
};
  ...
  tableData.value = tableData.value.map((tbs: any) => {
    ...
    if (selects.includes(tbs.sku)) {
      obj = {
        ...
        productCostIncludingTax: info?.productCostIncludingTax || obj.productCostIncludingTax
      };
      obj = {
        ...obj,
        // finishedProductCost: Number(Number((obj.productCostIncludingTax || 0)) + Number((obj.packagingManpower || 0))).toFixed(3),
      }
    }
    return obj;
  });
