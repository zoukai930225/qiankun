    ...
    >
      <template #tag v-if="listArr?.length">
        <div class="itemValue" v-for="(item, index) in listArr" :key="index">
          ...
          <span class="itemDesc"><!-- @vue-ignore -->{{ item?.name || item?.userName }} </span>
          <img
            v-if="isEdit"
            class="itemClose"
...
const getUserInfoDetail = (userId: string) => {
  let userInfo: Operator | null = null
...
const handleSelectChange = (value) => {
  ...
  value.forEach((item) => {
    ...
    if (typeof item === 'string') {
      ...
      userInfo = getUserInfoDetail(item.userId || item.chosenSupplierStaffId || '')
    }
    console.log('选择人员22:', item, userInfo)
    if (userInfo) {
      if (!props.multiple) {
        lists = [userInfo]
      } else {
