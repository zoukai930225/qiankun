    ...
    <div class="table-row" v-for="(row, index) in tableData" :key="index" :class="getRowClass(row)">
      ...
      >
        <div class="cell-content">
          <!-- 企业负责人 -->
          <div v-if="item.props == 'enterprisePlanManager'" class="user-info-wapper">
            <div class="user-info">
              <img :src="item.text.avatarOrigin" alt="" class="avator" />
              <span class="name">{{ item.text.name }}</span>
            </div>
          </div>

          <!-- 产研负责人 -->
          <div v-else-if="item.props == 'productionResearchHead'" class="user-info-wapper">
            <div class="user-info">
              <img :src="item.text.avatarOrigin" alt="" class="avator" />
              <span class="name">{{ item.text.name }}</span>
            </div>
          ...
          <!-- 选供负责人 -->
          <div v-else-if="item.props == 'chosenSupplierStaff'" class="user-info-wapper">
            <div class="user-info">
              <img
                :src="item.text.avatarOrigin"
                alt=""
                class="avator"
                v-if="item.text.avatarOrigin"
              />
              <span class="name" v-if="item.text.name">{{ item.text.name }}</span>
            </div>
          </div>
          <!-- 采购负责人 -->
          <div v-else-if="item.props == 'purchaseManagers'" class="user-info-wapper">
...
const tableData = computed(() => {
  ...
  return [
    [
      ...
      {
        text: data.enterprisePlanManager || [],
        isLabel: false,

        props: 'enterprisePlanManager'
      },
      { text: '产研负责人', isLabel: true },
      {
        text: data.productionResearchHead || [],
        isLabel: false,
        props: 'productionResearchHead',
        merged: true,
        colspan: 3 // 标记合并3个单元格
      }
    ],
    [
      ...
      {
        text: data.chosenSupplierStaff || '12%',
        isLabel: false,
        props: 'chosenSupplierStaff'
      },
      { text: '采购负责人', isLabel: true },
      {
        text: data.purchaseManagers || '是',
        props: 'purchaseManagers',
        isLabel: false,
        merged: true,
        colspan: 3 // 标记合并3个单元格
      }
    ...
    [
      ...
      { text: props.productCode || '', isLabel: false },
      { text: '供应商', isLabel: true, bgColor: 'rgba(234, 0, 0, 0.1)' },
      {
        text: props.fullCompany || '',
        isLabel: false,
        merged: true,
        colspan: 3 // 标记合并3个单元格
      }
