    ...
    <div class="table-row" v-for="(row, index) in tableData" :key="index" :class="getRowClass(row)">
      <div v-for="(item, itemIndex) in row" :key="itemIndex" class="table-cell" :class="{
        ...
        <div class="cell-content">
          <div v-if="item.props == 'quotationSheetFile'">
...
const tableData = computed(() => {
  ...
  return [
    [
      { text: '实物报价', isLabel: true },
      { text: data.physicalQuotePrice || '', isLabel: false },
      { text: '报价单', isLabel: true },
      {
        text: data.quotationSheetFile || '',
        isLabel: false,
        merged: true,
        props: 'quotationSheetFile',
        colspan: 3 // 标记合并3个单元格
      }
    ],
    [
