<template>
  <div class="treeNodeDetail">
    ...
    <ElScrollbar v-if="nodeList && nodeList.length > 0">
...
import TreeNode from '@/views/OA/package/components/treeNode.vue'
import { ElScrollbar } from 'element-plus'
import { useRouter } from 'vue-router'

import { queryProgressBoardByEnterprise } from '@/api/oa/package/develop'
...
const developPlanId = route.params.developPlanId
...
const getProgressDetial = async () => {
  ...
  const res = await queryProgressBoardByEnterprise({
    enterprisePlanId: developPlanId,
    taskId: taskId
  }).catch(() => {
...
const formatArr = (arr) => {
  // 将数组中的每个对象的nextProgress属性转换为children字段 需要递归替换
  return arr.map((item) => {
    return {
      ...item,
      children: item.nextProgress ? formatArr(item.nextProgress) : []
    }
  })
}
