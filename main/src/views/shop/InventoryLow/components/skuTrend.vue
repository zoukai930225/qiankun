<template>
  <div>
    ...
    >
      <template #title>
        <span style="color: #333; font-weight: 500; font-size: 14px">销量趋势 </span>
      </template>
...
import { getSkuThirtyData } from '@/api/shop/InventoryLow/index'
...
const areaColor: string[] = ['rgba(49, 111, 255, 0.5)', 'rgba(49, 111, 255, 0)']
const xName: string = '日期'
const yName: string = '每日销量'
const gridData = {
  left: 20,
  right: 40,
  bottom: 12,
...
interface GetSkuThirtyDataProps {
  productNumber: string
  goodsCode: string
}
/** 打开弹窗 */
const open = async (params: GetSkuThirtyDataProps) => {
  dialogVisible.value = true
  currentRef.value = params
  getList(params)
}
defineExpose({ open }) // 提供 open 方法，用于打开弹窗

const showChart = ref(true)
const getList = async (params: GetSkuThirtyDataProps) => {
  const data = await getSkuThirtyData(params)
...
const handleReqData = (data: any[]) => {
  ...
  lineOption.value.tipsName = '每日销量'
