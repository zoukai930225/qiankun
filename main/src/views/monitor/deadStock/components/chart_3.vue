  ...
  <div class="chart-part">
    <div ref="chartRef" class="chart-container">
      ...
      <div class="sum-up m-auto">
        <div class="sum-item">总销量</div>
        <div class="sum-item">{{ yesterdaySales }}</div>
        <div class="sum-item">剩余总库存</div>
        <div class="sum-item">{{ inventory }}</div>
      </div>
    </div>
  </div>
</template>
...
const yesterdaySales = ref('0 万')
const inventory = ref('0 万')

// 图表数据
const salesData = ref<number[]>([]) // 销量
const dailySalesData = ref<number[]>([]) // 每日剩余库存
...
const getChartOption = () => {
  const yAxis1Config = calculateYAxisConfig(salesData.value)
  const yAxis2Config = calculateYAxisConfig(dailySalesData.value)

  return {
    title: {
      text: `${props.date} 销量统计图`,
      textAlign: 'center',
      left: '50%',
...
const fetchChartData = async () => {
  try {
    ...
    yesterdaySales.value = '0.00 万'
    inventory.value = '0.00 万'

    if (!response) return message.warning(`${props.date} 未查询到数据`)

    const { histogramData, yesterdaySales: total, inventory: yesterday } = response

    if (histogramData?.length) {
      salesData.value = histogramData.map(
        (item) => Math.round(Number(item.yesterdaySales || 0) / 100) / 100
      )
      dailySalesData.value = histogramData.map(
        (item) => Math.round(Number(item.inventory || 0) / 100) / 100
      )
    }
    yesterdaySales.value = formatLargeNumber(total || 0)
    inventory.value = formatLargeNumber(yesterday || 0)
    await nextTick()
    renderChart()
  } catch (error) {
