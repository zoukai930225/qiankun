...
const config = computed(() => (props.config || chartsBars.find((ele) => ele.type === props.type))!)
...
async function getList() {
  try {
    ...
    const params = {
      ...queryParams,
      ...props.otherParams,
      goodsCodeQuery: queryParams.goodsNo ? queryParams.goodsNo.split(',') : undefined
    }
    const res = await config.value?.detailApi?.(params)
