    ...
    <template #prePart>
      ...
      <el-row :gutter="24">
        ...
        <el-col :span="4" class="!flex justify-end min-w-350px">
          ...
          <el-button
            v-if="permissionSet.has('SupplierDeliveryRate:Category:Export')"
...
import {
  exportDeliveryAnalysisSupplierApi,
  supplierDeliveryRateApi
...
const queryParams = ref({
  supplierIds: '' as string | undefined,
  months: (() => {
...
const supplierIds = computed({
  get: () =>
    queryParams.value.supplierIds ? queryParams.value.supplierIds?.split(',') : undefined,
  set: (val) => (queryParams.value.supplierIds = val && val.length ? val.join(',') : undefined)
})
...
const autoColumns = computed(() => {
  ...
  autoColumnsSet.delete('supplierId')
  autoColumnsSet.delete('supplierName')
  return Array.from(autoColumnsSet)
})
const tableOptions = computed<TableOption>(() => {
  return [
    { type: 'index', label: '序号', width: 60, fixed: 'left' },
    { label: '供应商名称', prop: 'supplierName', minWidth: 150, fixed: 'left', formatter: copyBtn },
...
function getList() {
  ...
  timer.value = setTimeout(async () => {
    try {
      const res = await supplierDeliveryRateApi(queryParams.value)
...
function handleCheck(row, month) {
  detailDrawerProp.value = { modelValue: true, ...row[month], code: row.supplierId, type: '1' }
}

const exportLoading = ref(false)
async function handleExport() {
  exportLoading.value = true
  let fileName = `供应商交付率-${formatDate(new Date(), 'YYYYMMDDHHmmss')}.xlsx`

  try {
    const res = await exportDeliveryAnalysisSupplierApi({
      ...queryParams.value,
      maxSize: total.value
    })
