      ...
      <el-row :gutter="24">
        <el-col :span="20" class="!flex-1">
          <el-form :model="queryParams" ref="formRef" label-width="90">
            <el-row :gutter="14">
              <el-col :span="6">
                <el-form-item label="品类" prop="categoryCodes">
                  <scroll-select
...
import {
  categoryDeliveryRateApi,
  exportDeliveryAnalysisCategoryApi,
  getCategorySelectorApi
...
const queryParams = ref({
  ...
  pageNum: 1,
  pageSize: 10,
  ...Object.fromEntries(SORT_COLUMN.map((ele) => [`${ele}Query`, { sort: null }]))
})
const total = ref(0)
...
const autoColumns = computed(() => {
  ...
  autoColumnsSet.delete('categoryName')
  autoColumnsSet.delete('categoryCode')
  return Array.from(autoColumnsSet)
})
const tableOptions = computed<TableOption>(() => {
  return [
    { type: 'index', label: '序号', width: 60, fixed: 'left' },
    { label: '品类', prop: 'categoryName', minWidth: 110, fixed: 'left' },
...
function getList() {
  ...
  timer.value = setTimeout(async () => {
    try {
      const res = await categoryDeliveryRateApi(queryParams.value)
...
function handleCheck(row, month) {
  detailDrawerProp.value = { modelValue: true, ...row[month], code: row.categoryCode, type: '2' }
}

const exportLoading = ref(false)
async function handleExport() {
  exportLoading.value = true
  let fileName = `品类交付率-${formatDate(new Date(), 'YYYYMMDDHHmmss')}.xlsx`

  try {
    const res = await exportDeliveryAnalysisCategoryApi({
      ...queryParams.value,
      maxSize: total.value
    })
