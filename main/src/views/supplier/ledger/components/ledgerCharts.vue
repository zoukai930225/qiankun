...
const inCounts = computed(() => chartsData.value.map((ele) => ele.inCount) || [0])
const inAmounts = computed(() => chartsData.value.map((ele) => ele.inAmount) || [0])
  ...
  const value = [
    {
      ...
      data: inCounts.value.length === 0 ? [0] : inCounts.value,
      yAxisIndex: 0,
    ...
    },
    {
      ...
      data: inAmounts.value.length === 0 ? [0] : inAmounts.value,
      yAxisIndex: 1,
...
const getChartOption = computed(() => {
  const yAxis1Config = calculateYAxisConfig(inCounts.value)
  const yAxis2Config = calculateYAxisConfig(inAmounts.value)
  return {
    title: {
      textAlign: 'center',
      left: '50%',
...
const handleLegendSelect = ({
  ...
  activeSeries.forEach((s: any) => {
    switch (s.name) {
      case '入库量':
        allData.push(...inCounts.value)
        break
      case '入库金额':
        allData.push(...inAmounts.value)
        break
    }
  })
...
function handleDatazoom(params) {
  ...
  const visibleInCounts = [...inCounts.value].slice(startIndex, endIndex + 1)
  const visibleInAmounts = [...inAmounts.value].slice(startIndex, endIndex + 1)
