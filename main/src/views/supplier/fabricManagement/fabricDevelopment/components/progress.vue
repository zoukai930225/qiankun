...
const visible = ref<boolean>(false), cardHeight = ref<number>(300), bodyHeight = ref<number>(300), supplierData = ref<any>({}), advanceRef = ref(), loading = ref<boolean>(false);

const dataItem = ref<any>({}), dataValue = ref<any>({ inProgress: true, list: [] }), isSecond = ref<boolean>(false), quoteRef = ref(), quoteValue = ref<Item>({ ...new Item() });

const title = computed(() => (!isSecond.value ? '打样进度' : `${supplierData.value?.supplierName || ''}-报价核价`));

const getDataInfo = (fun: Function) => {
  request.get({ url: `/workflow-api/api/workflow/business/factorySecondary/supplierInfo/list`, params: { fsTaskId: dataItem.value.id, inProgress: dataValue.value.inProgress } }).then((res: any) => {
...
const quote = async (item: any) => {
  ...
  const supplierInfo: any = await request.get({ url: `/workflow-api/api/workflow/business/factorySecondary/supplierInfo/queryFsSupplierQuoteVerification`, params: { supplierInfoId: supplierData.value.id } });
  supplierInfo?.fsTaskId && (quoteValue.value = { ...new Item(), ...supplierInfo });
  const colorInfo: any = await request.get({ url: `/workflow-api/api/workflow/business/factorySecondary/supplierInfo/queryWaitQuotePriceList/${dataItem.value.id}/${supplierData.value.supplierId}` });
  colorInfo?.records?.length && (quoteValue.value = { ...quoteValue.value, colorSizeGroups: colorInfo.records });
...
const submit = async () => {
  ...
  if (check) {
    if (!!quoteValue.value.colorSizeGroups?.length) {
      ...
      loading.value = true;
      request.post({ url: `/workflow-api/api/workflow/business/factorySecondary/supplierInfo/confirmQuotePrice`, data: { ...quoteValue.value, id: supplierData.value.id, fsTaskId: dataItem.value.id, supplierId: supplierData.value.supplierId } }).then((res: any) => {
