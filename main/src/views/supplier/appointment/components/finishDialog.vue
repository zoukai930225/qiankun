    ...
    <el-form :model="formData" ref="formRef" :rules="rules" label-width="100">
      ...
      <el-form-item label="附件：" prop="attachmentUrls">
        <el-upload
          ...
          multiple
          name="files"
          accept=".xlsx,.xls,image/*,.txt,.pdf,.doc,.docx"
          :action="importUrl"
...
import { scAdvancedShippingRequestOrderFinishApi } from '@/api/supplier/appoint'
...
const rules: FormRules = {
  comment: { required: true, message: '请输入结单说明' }
}
...
async function handleSubmit() {
  ...
  try {
    ...
    const res = await scAdvancedShippingRequestOrderFinishApi({
      ...formData,
      asrId: props.asrId || ''
    })
    if (res.success) {
...
function handleRemove(file: UploadFile & UploadRawFile) {
  ...
  const { data } = file.response as any
  if (data) {
    fileListMap.delete(data[0].fileName)
  }
