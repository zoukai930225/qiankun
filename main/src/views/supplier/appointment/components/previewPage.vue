  ...
  <div id="preview" class="print">
    ...
    <div class="content">
      ...
      <div class="row_2">
        ...
        <div class="row_2_content cell">{{ printInfo.receivingWarehouseVo?.contactNumber }}</div>
      </div>
      <div class="row_2">
      ...
      <div class="row_6">
        ...
        <div class="row_6_item row_6_itemitem_7 cell">箱数</div>
      ...
      <div class="row_7">
        ...
        <div class="row_7_item row_6_itemitem_7 cell">{{ boxNumberTotal }}</div>
      ...
      <div class="row_8" v-for="(item, index) in printInfo.packingList" :key="item.id">
        ...
        <div class="row_8_item row_6_itemitem_6 cell" :style="row8Style(item, index, 'boxNumber')">
          {{ item.boxNumber }}
        </div>
        <div class="row_8_item row_6_itemitem_7 cell">{{ item.boxQuantity }}</div>
        <div
          class="row_8_item row_6_itemitem_7 cell"
          :style="row8Style(item, index, 'boxNumber')"
          >{{ (item.endBoxNumber ?? 0) - (item.startBoxNumber ?? 0) + 1 }}</div
        >
...
const boxNumberTotal = computed(() => {
  const numSet = new Set()

  props.printInfo.packingList?.forEach((row) => {
    if (row.startBoxNumber === row.endBoxNumber) numSet.add(row.startBoxNumber)
    else {
      for (let i = row.startBoxNumber; i! <= row.endBoxNumber!; i!++) {
        numSet.add(i)
      }
    }
  })
  return Array.from(numSet.values()).length
})
...
function isFirstRowOfPage2(index) {
  const { fullPageRowNum, noFullPageRowNum } = ROW_CONFIG
  return (
    index + fullPageRowNum - noFullPageRowNum === fullPageRowNum * getPageIndex(index) &&
    index !== 30
  )
}
...
onUpdated(() => {
  const cells = document.querySelectorAll('#preview .row_8')
  cells.forEach((ele, index) => {
    if (isFirstRowOfPage2(index)) {
