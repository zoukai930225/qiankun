...
/** 打开弹窗 */
const open = async (storeRangeStr: string, type: string, singleSelect = false) => {
  console.log('打开弹窗,', storeRangeStr)
...
const updateSelectedStores = (storeRangeStr, type) => {
  ...
  if (storeRangeStr !== '全渠道') {
    ...
    for (const platform in platformData.value) {
      platformData.value[platform].forEach((shop) => {
        ...
        if (shopNamesArray.includes(valueToMatch)) {
          ...
          currentIndex.value = Object.keys(platformData.value).findIndex((el) => el === platform)
    ...
    console.log('cc', checkList.value)
  } else {
    checkList.value = []
  }
}
...
const storeData = ref<any>([]) // 店铺数据
const getQueryChannels = async () => {
  ChannelLoading.value = true
  try {
    const data = await TagMngApi.getQueryChannels().catch(() => {})
    const categorizedData = transformData(data)

    platformData.value = categorizedData
    // 在第一条添加一条数据,用于展示"全渠道"的选项
    if (!props.showAllChannel) {
      // 不需要展示全渠道
      platformData.value = {
        ...categorizedData
      }
      getStoreList(Object.keys(platformData.value)[0])
    } else {
      platformData.value = {
        ...{ 全渠道: [] }, // 首先添加"全渠道"的条目
...
const handleSubmit = () => {
  ...
  if (currentIndex.value === 0 && props.showAllChannel) {
