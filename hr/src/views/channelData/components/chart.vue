...
const setChartOptions = () => {
  ...
  let tooltip = {
    ...
    formatter: function (param) {
      if (param.length > 1) {
        return (
          '<div style="min-width: 150px ;padding-right: 6px;height: 70px;background: #ffffff;border-radius: 4px;display: flex;flex-direction: column;margin:-4px; ">' +
        ...
        return (
          '<div style="width: 150px;padding-right: 6px;height: 50px;background: #ffffff;border-radius: 4px;display: flex;flex-direction: column;margin:-4px; ">' +
          `<div style="display: flex;flex-direction: row;align-items: center;padding-top: 4px;margin-left:6px;font-size: 14px;color: #8186A5;">` +
          '<div>' +
          `${param[0].axisValue}` +
          '</div>' +
          ...
          `<div style="display: flex;flex-direction: row;align-items: center;padding-top: 0px;margin-left:6px;font-size: 14px;color: #333;">` +
          `<div style="padding-right:5px">` +
          `${param[0].seriesName === '点击率' ? props.option.tipsName : props.option1.tipsName}` +
          '</div>' +
          `<div style="width: 6px;height: 6px;background: ${param[0].seriesName === '点击率' ? props.option.lineColor : props.option1.lineColor};border-radius:3px;margin-right:4px;"></div>` +
          `<div>` +
          `${Number(param[0].value)}%` +
          '</div>' +
          '</div>'
  ...
  series[0] = {
    ...
    showSymbol: true,
    symbolSize: 0, // 拐点大小
    connectNulls: false, // 连接空值，防止线条断开
    data:
