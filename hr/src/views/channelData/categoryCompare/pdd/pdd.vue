  ...
  <div class="mainContainer">
    ...
    <div class="mainContainer-content">
      <div class="mainContainer-content-left">
        <div>
          <CategoryItemPdd ref="allChargeRef" type="total" :list="allChargeList" />
        </div>
        <div style="margin-top: 20px">
          <CategoryItemPdd ref="paymentRef" type="sale" :list="paymentList" />
        </div>
        <div style="margin-top: 20px">
          <CategoryItemPdd ref="refundRef" type="refund" :list="refundList" />
        </div>
        <div style="margin-top: 20px">
          <CategoryItemPdd ref="refundRatioRef" type="refundRatio" :list="refundRatioList" />
        </div>
        <div style="margin-top: 20px">
          <CategoryItemPdd ref="refundRateRef" type="refundRate" :list="refundRateList" />
        </div>
      </div>
      <!-- <div class="mainContainer-content-right"> </div> -->
      <ElScrollbar class="mainContainer-content-right">
        <CategoryRemark ref="categoryRemarkRef" @edit-click="editClick" />
      </ElScrollbar>
    </div>
    ...
    <div ref="exportImgMainRef" class="exportMainContainer">
      <div class="exportMainContainer-content">
        <div class="exportMainContainer-content-left">
          <div style="margin-top: 20px">
            <CategoryItemPdd ref="exportAllChargeRef" type="total" :list="allChargeList" />
          </div>
          <div style="margin-top: 20px">
            <CategoryItemPdd ref="exportPaymentRef" type="sale" :list="paymentList" />
          </div>
          <div style="margin-top: 20px">
            <CategoryItemPdd ref="exportRefundRef" type="refund" :list="refundList" />
          </div>
          <div style="margin-top: 20px">
            <CategoryItemPdd
          ...
          <div style="margin-top: 20px">
            <CategoryItemPdd ref="exportRefundRateRef" type="refundRate" :list="refundRateList" />
          </div>
        </div>
        <div class="exportMainContainer-content-right">
          <CategoryRemark ref="exportCategoryRemarkRef" />
        </div>
      </div>
    </div>
  </div>
...
import CategoryItemPdd from './components/categoryItem.vue'
import CategoryRemark from './components/categoryRemark.vue'
...
const getList = async () => {
  ...
  try {
    ...
    const data = await ChannelDataApi.getStoreComparisonAndAnalysisList(params)
    if (data) {
      // tghf 付费总和 xse 销售额 tkje 退款金额 qtfb 去退费比 tkl 退款率
      // svcList.value = data.svc || []
      allChargeList.value = formatList(data.storeStats.tghf) || []
      paymentList.value = formatList(data.storeStats.xse, 'paymentAmount') || []
      refundList.value = formatList(data.storeStats.tkje, 'refund') || []
      refundRateList.value = formatList(data.storeStats.tkl, 'refundRate') || []
      refundRatioList.value = formatList(data.storeStats.qtfb, 'refundRatio') || []
    }
    nextTick(() => {
      // svcRef.value?.handleData(svcList.value)

      console.log('-------', allChargeList.value)
...
const formatList = (data, fieldName = 'charge') => {
  // 首先收集所有可能的categoryName
  ...
  const sortedKeys = Object.keys(data).sort((a, b) => {
    ...
    const aIsDate = dateRegex.test(a)
    const bIsDate = dateRegex.test(b)
    ...
    // 如果都不是日期，按特殊规则排序
    if (!aIsDate && !bIsDate) {
      const aHasHuanbi = a.includes('环比')
      const bHasHuanbi = b.includes('环比')

      // 包含"环比"的放最后
      if (aHasHuanbi && !bHasHuanbi) return 1
      if (!aHasHuanbi && bHasHuanbi) return -1
  ...
  for (const key of sortedKeys) {
    ...
    if (typeof value === 'object' && value !== null) {
      ...
      // 判断是否为环比数据，如果是则使用 'qoq' 作为字段名
      const actualFieldName = key.includes('环比') ? 'qoq' : fieldName

      // 处理新格式：数组形式的数据
      if (Array.isArray(value)) {
        // 创建一个映射来存储每个category的数据
        const categoryMap = new Map()

        for (const item of value) {
          for (const [itemKey, itemValue] of Object.entries(item)) {
            if (itemKey !== 'percentage') {
              const percentage = itemKey === '合计' ? 0 : parseFloat(item.percentage || '0')
        ...
        for (const category of sortedCategoryNames) {
          ...
          const entry = {
            category: category,
            [actualFieldName]: categoryData ? categoryData.value : '0',
            percentage: categoryData ? categoryData.percentage : 0
          }
          dateEntry.value.push(entry)
        }
      } else {
        // 处理旧格式：对象形式的数据（保持兼容性）
        for (const category of sortedCategoryNames) {
          const entry = {
            category: category,
            [actualFieldName]: value[category] || '0',
            percentage: 0
          }
          dateEntry.value.push(entry)
        }
...
const exportImg = async () => {
  ...
  nextTick(async () => {
    try {
      ...
      downloadLink.download = 'SW拼多多店铺对比'
