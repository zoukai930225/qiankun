<template>
  ...
  <div class="mainContainer">
    ...
    >
      ...
      <!-- <el-button
        style="margin-left: auto; margin-right: 5px; margin-top: -10px"
        type="primary"
        @click="getList()"
      >
    ...
    <div class="mainContainer-content">
      <div class="mainContainer-content-left">
        <CategoryItem ref="svcRef" type="search" :list="svcList" />
        <div style="margin-top: 20px">
          <CategoryItem ref="allChargeRef" type="total" :list="allChargeList" />
        </div>
        <div style="margin-top: 20px">
          <CategoryItem ref="paymentRef" type="sale" :list="paymentList" />
        </div>
        <div style="margin-top: 20px">
          <CategoryItem ref="refundRef" type="refund" :list="refundList" />
        </div>
        <div style="margin-top: 20px">
          <CategoryItem ref="refundRatioRef" type="refundRatio" :list="refundRatioList" />
        </div>
        <div style="margin-top: 20px">
          <CategoryItem ref="refundRateRef" type="refundRate" :list="refundRateList" />
        </div>
      </div>
      <!-- <div class="mainContainer-content-right"> </div> -->
      <ElScrollbar class="mainContainer-content-right">
        <CategoryRemark ref="categoryRemarkRef" @edit-click="editClick" store="swTaobao" />
      </ElScrollbar>
    </div>
    ...
    <div ref="exportImgMainRef" class="exportMainContainer">
      <div class="exportMainContainer-content">
        <div class="exportMainContainer-content-left">
          <CategoryItem ref="exportSvcRef" type="search" :list="svcList" />
          <div style="margin-top: 20px">
            <CategoryItem ref="exportAllChargeRef" type="total" :list="allChargeList" />
          </div>
          <div style="margin-top: 20px">
            <CategoryItem ref="exportPaymentRef" type="sale" :list="paymentList" />
          </div>
          <div style="margin-top: 20px">
            <CategoryItem ref="exportRefundRef" type="refund" :list="refundList" />
          </div>
          <div style="margin-top: 20px">
            <CategoryItem ref="exportRefundRatioRef" type="refundRatio" :list="refundRatioList" />
          </div>
          <div style="margin-top: 20px">
            <CategoryItem ref="exportRefundRateRef" type="refundRate" :list="refundRateList" />
          </div>
        </div>
        <div class="exportMainContainer-content-right">
          <CategoryRemark ref="exportCategoryRemarkRef" store="swTaobao" />
        </div>
      </div>
    </div>
  </div>
...
const exportImg = async () => {
  ...
  nextTick(async () => {
    try {
      ...
      downloadLink.download = 'SW淘宝店品类对比'
