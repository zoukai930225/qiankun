    ...
    <el-option v-for="item in options" :key="item.id" :label="item.name" :value="item.id"
      ...
      <div class="peopleOption">
        <div class="left-wapper">
          <img class="img" :src="item?.tx || avatarLogo" alt="" />
          <div class="info">
...
import { getAssetConfirmationUsers } from '@/api/hrAdmin/assetStatistics'
...
const focusOn = () => {
  JSON.stringify(props.feedBackOptions) === JSON.stringify(options.value) && getDataList();
};

const getDataList = () => {
  getAssetConfirmationUsers({ }).then((res: any) => {
    if (res?.length) {
      const userIds = res.map((item) => item.id);
...
const getLableAvater = (userId: string) => {
  return options.value.find((item) => item.id === userId)?.avatarOrigin || avatarLogo
}
