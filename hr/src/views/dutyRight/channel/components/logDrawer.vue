<template>
  <scheme-details ref="detailsRef" name="ChannelLog" :width="1000">
    <template #card>
      <div>
        <el-form inline :model="dataValue" label-width="auto">
          <el-form-item style="margin-right: 20px;">
            <el-select v-model="dataValue.bizId" placeholder="请选择渠道" filterable clearable @change="handleQuery"
              style="width: 200px;">
              <el-option v-for="item in getIntDictOptions(DICT_TYPE.SYSTEM_PARAM_CHANNEL, true)" :key="item.value"
        ...
        <el-table :header-cell-style="appStore.headerCellStyle" v-loading="loading" :data="tableList"
          ...
          <el-table-column prop="bizId" label="渠道" width="150">
            <template #default="{ row }">
              <ChannelCell :row="row" keyName="bizId" />
            </template>
          </el-table-column>
...
<script lang="tsx">
export default { name: 'ChannelLog' };
</script>
...
class DataItem {
  ...
  size: number = 10;
  bizType: number = 0;
  bizId: string | undefined = undefined;
...
const getList = async () => {
  loading.value = true
  try {
    const data = await request.get({ url: "/api/opDutyAuthorityManage/recordList", params: dataValue.value })
    if (data) {
...
provide('ChannelLogDetails', { dataValue, bukValue, viewType, title });
