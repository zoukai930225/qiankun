...
<template>
  <div class="select-wapper">
    <el-select popper-class="eloption" :popper-append-to-body="true" v-model="listArr" placeholder="请选择"
      :multiple="true" remote @change="handleSelectChange" filterable :remote-method="remoteMethod">
      <template #tag v-if="listArr?.length">
        <div class="itemValue" v-for="(item, index) in listArr" :key="index">
          ...
          <span class="itemDesc">{{ formatName(item, 'name') }}</span><img class="itemClose"
            src="@/assets/imgs/team/itemClose.png" @click.stop="delItem(item, 'operate')" alt="" />
        </div>
      </template>
      <el-option v-for="dict in OperatorRosterList" :key="dict.userId" :label="dict.operatorRealname"
        :value="dict.userId">
        <div class="item">
...
const listArr = defineModel<any[]>({ default: [] })
const operatorName = defineModel<string | null>('operatorName')
const userId = defineModel<string | null>('userId')
const directorAvatar = defineModel<string | null>('directorAvatar')
const emits = defineEmits(['change'])


function changeModule(value: any) {
  listArr.value = value
  operatorName.value = OperatorRosterList.value.find((el) => el.userId === value[value.length - 1])?.name
  userId.value = OperatorRosterList.value.find((el) => el.userId === value[value.length - 1])?.userId
  directorAvatar.value = OperatorRosterList.value.find((el) => el.userId === value[value.length - 1])?.avatarOrigin
