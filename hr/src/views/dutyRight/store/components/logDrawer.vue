<template>
  <scheme-details ref="detailsRef" name="StoreLog" :width="1000">
    <template #card>
      <div>
        <el-form inline :model="dataValue" label-width="auto">
          <el-form-item style="margin-right: 20px;">
            <StoreCascader style="width: 200px;" v-model="dataValue.bizId" @change="handleQuery"
        ...
        <el-table :header-cell-style="appStore.headerCellStyle" v-loading="loading" :data="tableList"
          ...
          <el-table-column prop="bizId" label="店铺" width="180">
            <template #default="{ row }">
              <storeCell :row="row" channelName="channel" />
            </template>
          </el-table-column>
...
const getList = async () => {
  ...
  try {
    ...
    params.bizId = dataValue.value.bizId?.length > 0 ? dataValue.value.bizId[dataValue.value.bizId.length - 1] : '';
    const data = await request.get({ url: "/api/opDutyAuthorityManage/store/recordList", params: params })
    if (data) {
