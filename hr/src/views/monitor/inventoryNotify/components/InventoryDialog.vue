    ...
    <el-dialog v-model="visible" title="预警处理" @close="closeDialog" width="600">
      <el-form class="dialogForm" ref="formRef" v-loading="formLoading" :model="formData" label-width="110px"
        ...
        <el-form-item label="处理说明" prop="remarks">
          <s-text-area v-model="formData.remarks" placeholder="请输入处理说明" :autosize="{ minRows: 5 }" :maxlength="200" />
        </el-form-item>
        <el-form-item label="附件" prop="attachmentUrl">
          <div class="upload">
            <el-upload v-model:file-list="fileList" :action="importUrl" class="upload" :headers="uploadHeaders"
              name="files" :show-file-list="false" :data="{ code: 'NEW_PRODUCT', businessForeignKey: ids[0] }"
...
async function submitForm() {
  ...
  try {
    ...
    const data = await MonitorApi.wdtInventoryDealsSaveInBatch(formData)
    if (data.success) {
