      ...
      <template #basicInfo>
        <el-row>
          ...
          <el-col :span="12">
            ...
            >
              <template #label>
                <span class="label-text">测图计划<br />状态</span>
              </template>
              <el-input
              ...
              <el-col :span="24">
                ...
                >
                  <Transfer
                    ...
                    value-type="array"
                    v-model="detail.imgs"
                    :uploadData="{ code: UPLOAD_FILE_BISSONESSCODE.PICTURE_GALLERY_PROMOTION }"
                  />
                </el-form-item>
              </el-col>
            </el-row>
...
const getPromotionImageList = (imgList: any[] = []) => {
  ...
  if (imgList && imgList.length > 0) {
    imgList.forEach((item: any) => {
      // Transfer 组件 array 模式返回的对象包含 url 字段
...
const submit = async () => {
  ...
  const payload = (promoteForm.value.versionList || []).map((detail: any) => {
    const fileList = Array.isArray(detail?.imgs)
      ? detail?.imgs.map((item: any) => {
          return {
...
.sw-upload-file {
  ...
  // Transfer 组件内部样式通常足够，以下样式可根据 Transfer 实际渲染情况决定是否保留
