...
import { DICT_TYPE, getDictValue, getIntDictOptions, getDictLabel } from '@/utils/dict'
...
function upDateInfo(quantity: number, row: StorageSalary.Examien.DetailVos) {
  const target = formData.value.find((item) => item.rolesMask === row.rolesMask)!
  if (!target.updateSalaryDetailVos) target.updateSalaryDetailVos = []
  target.updateSalaryDetailVos.push({ ...row, quantity })

  const tableTarget = infos.value
    .find((item) => item.rolesMask === row.rolesMask)
    ?.detailVos?.find((item) => item.classificationId === row.classificationId)
  tableTarget && (tableTarget.quantity = quantity)
}
