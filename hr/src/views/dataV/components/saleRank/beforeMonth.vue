<template>
  <ElScrollbar>
    <div class="bg">
      ...
      <div class="bg-title">
        <img class="bg-title-left" src="@/assets/imgs/dataV/sale/month_rank.png" />
        ...
        >
          ...
          >
            <div class="item-group-value">{{
              index < beforeMonthTopRankGropuList().length
                ? beforeMonthTopRankGropuList()[rankIndex(index)]
                : ''
            }}</div>
          </div>
        </div>
              ...
              >
                ...
                >
                  ...
                  <div v-if="!value.group && !value.progress" class="item-value">
                    <div
                      v-if="value.range"
                      :style="{ color: Number(value.value) < 0 ? '#D80101' : '#333333' }"
                      >{{ value.value }}</div
                    >
...
const tableTopRankConfigList = () => {
  return beforeMonthTopRankList()
}

const tableColumnConfigList = (item) => {
  const extraWidth = Math.floor(96 / 13)
  ...
  const valueList = [
    ...
    {
      name: '分组',
      width: 126 + extraWidth,
      value: item.groupName || '',
      group: true
    },
    {
      name: '有效接待人数',
      width: 142 + extraWidth,
      value: item.effectiveReceptionCapacity || ''
    },
    {
      name: '加班人数',
      width: 110 + extraWidth,
    ...
    },
    {
      name: '转化率排名',
      width: 126 + extraWidth,
      value: item.conversionRateRank || ''
    },
    {
      name: '转化率系数',
      width: 126 + extraWidth,
      value: item.conversionRateCoefficient || ''
    },
    // {
    //   name: '质检分',
    //   width: 96,
...
const beforeMonthTopRankGropuList = () => {
  const list = []
  props.topList &&
    props.topList.forEach((item, index) => {
