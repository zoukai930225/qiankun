<template>
  <div class="goodCard">
    <div class="goodCard-bg">
      <div class="goodCard-bg-name" @click="clickGoodCard"
        >{{ good.materialName }}
        <!-- <a class="goodCard-bg-name" :href="good.goodsHref">{{ good.materialName }}</a> -->
        <img
          class="goodCard-bg-goodId-copy"
          src="@/assets/imgs/common/copy.png"
          alt=""
          @click.stop="copyGoodId"
        />
      </div>
...
const clickGoodCard = () => {
  ...
  // }
  window.open(props.good.goodsHref, '_blank')
}

const copyGoodId = async () => {
  try {
    if (props.good.materialName) {
      if (navigator.clipboard && window.isSecureContext) {
        await navigator.clipboard.writeText(props.good.materialName)
        message.success('复制成功')
      } else {
        // 创建text area
        let textArea = document.createElement('textarea')
        textArea.value = props.good.materialName
