...
const emit = defineEmits(['update:modelValue', 'update:operatorName', 'update:director'])
const handleSelectChange = (value) => {
  if (props.multiple) {
    ...
    if (value && value.length > 0) {
      ...
      // 设置责任人
      const name = OperatorRosterList.value.find(
        (el) => el.userId === value[value.length - 1]
      )?.name

      emit('update:director', { directorId: value[value.length - 1], directorName: name })
      nextTick(() => {
