<template>
  <ex-screen>
    <query-scheme :scheme="scheme" v-model="searchData" name="Edition" :show-expand="false" />
    <scheme-table :btn-list="btns" :handle-column="handleColumn" :screen="screen" name="Edition" :column="column"
      :table-data="dataList" :list-name="'版本列表'" />
    <edition-details ref="detailsRef" />
  </ex-screen>
</template>

<script lang="tsx">
export default { name: 'Edition' };
</script>

<script lang="tsx" setup>
import { PageItem, BtnItem } from '@/components/template/config/type';
import EditionDetails from "../details/index.vue";
...
const onSearch = () => {
  ...
  loading.value = true;
  request.get({ url: `/api/sysVersionUpdate/list`, params }).then((res: any) => {
...
provide('EditionForm', { searchData, dataList, page, loading, stateInfo, statistics, setParamsData, resetForm, onSearch, onButtonClick, viewDetails });
