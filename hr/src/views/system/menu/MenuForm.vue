  ...
  <Dialog v-model="dialogVisible" :title="dialogTitle">
    <el-form ref="formRef" v-loading="formLoading" :model="formData" :rules="formRules" label-width="100px">
      <el-form-item label="运行环境">
        <el-select v-model="formData.menuEnv">
          <el-option v-for="item in envOptions" :label="item.label" :value="item.value" />
        </el-select>
      </el-form-item>
      <el-form-item label="上级菜单">
        <el-tree-select v-model="formData.pId" :data="menuTree" :default-expanded-keys="['0']" :props="defaultProps"
      ...
      <el-form-item label="菜单类型" prop="type">
        <el-radio-group v-model="formData.type">
          <el-radio-button v-for="dict in MenuTypeOptions" :key="dict.label" :label="dict.value">
            {{ dict.label }}
          </el-radio-button>
        </el-radio-group>
      </el-form-item>
      <el-form-item v-if="formData.type === 1 && formData.menuEnv !== '2'" label="菜单图标" prop="icon">
        <IconSelect v-model="formData.icon" clearable />
      </el-form-item>
      <el-form-item v-if="formData.type === 2" label="应用图标" prop="icon">
        <transfer :upLoad-data="{ code: '' }" v-model="formData.icon" is-btn :is-drag="false" :tip="''" :limit="1"
          :is-joint="false" />
      </el-form-item>
      ...
      <el-form-item label="菜单状态" prop="status">
        <el-radio-group v-model="formData.status">
          <el-radio v-for="dict in CommonStatusOptions" :key="dict.label" :label="dict.value">
            {{ dict.label }}
          </el-radio>
        </el-radio-group>
      </el-form-item>
...
import { MenuTypeOptions, CommonStatusOptions } from '@/types/enumOptions'
...
const formRules = reactive({
  ...
  icon: [{ required: false, message: '菜单图标不能为空', trigger: 'blur' }],
...
/** 打开弹窗 */
const open = async (type: string, id?: string, parentId?: string, item?: any) => {
  ...
  item?.menuEnv && (formData.value = { ...formData.value, menuEnv: item.menuEnv });
  // 获得菜单列表
  await getTree()
}
  ...
  try {
    if (
      ...
      if (!isExternal(formData.value.path)) {
        if (formData.value.pId === '0' && formData.value.path.charAt(0) !== '/') {
          message.error('路径必须以 / 开头')
          return
        } else if (formData.value.pId !== '0' && formData.value.path.charAt(0) === '/') {
    ...
    if (formType.value === 'create') {
      data.id = undefined
      await MenuApi.createOrUpdateMenu(data)
...
const getTree = async () => {
  menuTree.value = []
  const data = await MenuApi.getMenuList({ menuEnv: formData.value.menuEnv })
  let menu: Tree = { id: '0', name: '主菜单', children: [] }
  menu.children = data || []
  menuTree.value.push(menu)
  if (ptId.value) {
    formData.value.pId = ptId.value
  }
}
