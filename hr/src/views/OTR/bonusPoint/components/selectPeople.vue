...
const handleSelectChange = (value) => {
  ...
  if (!props.isMultiple) {
    ...
    // 如果是多选
    const midArr = [...searchResult.value]
    const operatorArr = midArr.filter((item) => value.includes(item.userId))

    // 使用 Set 去重，基于 userId
    const uniqueOperators = Array.from(new Set(operatorArr.map((item) => item.userId))).map(
      (userId) => operatorArr.find((item) => item.userId === userId)
    )

    emit('change', uniqueOperators)
  }
}

onMounted(() => {
  searchResult.value = []
  OperatorRosterList.value = departmentStore.designerList || []
