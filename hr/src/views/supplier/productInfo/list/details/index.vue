          ...
          <el-row :gutter="20">
            ...
            <el-col :span="24">
              <el-form-item label="货品图片" prop="mainImgUrl">
                <update-img ref="imgRef" v-model:main-img-url="dataValue.mainImgUrl" @change="handleChangeImg" />
              </el-form-item>
            </el-col>
            <el-col :span="24">
            ...
            <el-col :span="12">
              <el-form-item label="原编码" prop="iterateStyle ">
                <el-input v-model="dataValue.iterateStyle" disabled placeholder="--" />
              </el-form-item>
            </el-col>
            <el-col :span="12">
              <el-form-item label="系列" prop="series">
                <el-input v-model="dataValue.series" disabled placeholder="--" />
              </el-form-item>
            </el-col>

            <el-col :span="12">
              <el-form-item label="品类" prop="category">
                <el-cascader style="width: 100%" v-model="category" :options="productInfoData.categoryOptions"
                  :props="{ ...customProps, multiple: false, checkStrictly: true }" placeholder="请选择货品品类" collapse-tags
            ...
            <el-col :span="12">
              <el-form-item prop="className" label="ERP品类">
                <el-input :model-value="dataValue.className" disabled placeholder="--" />
              </el-form-item>
            </el-col>

            <el-col :span="12">
              <el-form-item label="商品等级" prop="level">
                <el-select v-model="dataValue.level" placeholder="--" disabled>
            ...
            <el-col :span="12">
              <el-form-item label="类型" prop="goodsType">
                <el-select v-model="dataValue.goodsType" placeholder="--" disabled>
                  <el-option v-for="dict in getIntDictOptions(DICT_TYPE.NP_DESIGN_PRODUCT_TYPE, true)" :key="dict.value"
                    :label="dict.label" :value="dict.value" />
                </el-select>
              </el-form-item>
            </el-col>
            <el-col :span="12">
              <el-form-item label="安全类别" prop="securityTechnologyCategory">
                <el-select v-model="dataValue.securityTechnologyCategory" placeholder="--" disabled>
            ...
            <el-col :span="12">
              <el-form-item label="执行标准" prop="executionStandards">
                <el-select v-model="dataValue.executionStandards" placeholder="--" disabled>
            ...
            <el-col :span="12">
              <el-form-item label="跟单人员" prop="userIds">
                <el-select popper-class="eloption" :popper-append-to-body="true" v-model="dataValue.userIds"
            ...
            <!-- <el-col :span="12">
              <el-form-item label="标签价" prop="labelPrice">
                <el-input v-model="dataValue.labelPrice" />
              </el-form-item>
            </el-col> -->

            <el-col :span="12">
              <el-form-item label="货品供应商" prop="supplierIds">
                <el-select disabled v-model="dataValue.supplierIds" filterable multiple collapse-tags
...
const openView = (item: any = {}, type: string = 'view') => {
  ...
  detailsRef?.value?.openDrawer(async () => {
    ...
    console.log(item)
    if (item.goodsId) {
      request.get({url: `/api/sc/goods/getGoodsInfoById?goodsId=${item.goodsId}`}).then((data: any) => {
        if (data) {
          // 处理字段映射和默认值
          ...
          category.value = dataValue.value.category
...
const submit = debounce(async () => {
  ...
  const submitData = {
      ...dataValue.value,
      outOfStock: dataValue.value.outOfStock ?? 0,
      // 确保提交的是正确的字段名
      supplierIds: dataValue.value.supplierIds || [],
      userIds: dataValue.value.userIds || [],
      category:
  ...
  request.post({ url: `/api/sc/goods/updateGoods`, data: submitData }).then(() => {
