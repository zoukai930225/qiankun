<template>
  <div class="progressMainContent" v-loading="searchLoading" style="width: 100%;">
    ...
    <div style="width: 100%; min-width: 700px; position: relative; box-sizing: border-box">
      <div style="width: 100%; min-width: 1600px; display: flex">
        <div style="display: flex; width: 100%">
          <div class="left" style="overflow: hidden; position: relative; flex: 1">
            <div class="documents" style="padding-right: 0px; width: 209px; border-right: none">
              ...
              <el-collapse class="docCollapse" v-model="collapseItems">
                ...
                >
                  <fileItem
                    v-if="currentTaskDetails && doc.key !== 'fileUrl'"
            ...
            <div class="infos" style="width: calc(100% - 209px)">
              <infosTable
                v-for="(table, index) in newInfoTables"
                :key="index"
                :params="table"
                :infoData="currentTaskDetails"
                @success="getRefreshData"
              />
            </div>
          </div>
          <!-- 流程节点 -->
          <div
            class="right"
            style="background-color: #ffffff; width: 410px; position: relative; padding-right: 10px"
            :style="{ width: `${nodeShowWidth + 16}px` }"
            ref="resizeLeftHandle"
          >
            <div style="display: flex; position: relative">
              ...
              <div style="width: 410px; margin-left: 16px" :style="{ width: `${nodeShowWidth}px` }">
                ...
                <card-flow
                  ref="flowRef"
                  @changeSelectedNode="getSelectedNode"
                ...
                <div class="nodeForm" v-if="selectedNode?.taskDefinitionKey">
                  <img
                    @mousedown.stop.passive="startResize"
                    style="
                      width: 16px;
                      height: 16px;
                      cursor: move;
                  ...
                  <div class="formTitle">
                    ...
                    <div
                      class="nodeOperationBtn"
                      v-if="canAuditOrSubmit && showDialogContent && canOperate"
                    >
                  ...
                  <div class="formContent" style="padding: 0 10px">
                    <nodeForm
                      :data="currentNodeDetails"
                      :formData="currentNodeForm"
                      :disabled="!showBtns || isPreProductSampleNode || !canOperate"
                  ...
                  <div
                    class="formFooter"
                    style="margin: 0 10px"
                    v-if="showBtns && !isPreProductSampleNode && canOperate"
                  >
                    ...
                    <template
                      v-if="currentNodeDetails.taskDefinitionKey === 'FsProcurementConfirmation'"
                    >
                      <el-button
                        v-if="
                          ![0, 1].includes(currentNodeDetails?.extraData?.procurementConfirmStatus)
                        "
                        @click="openOrderForm"
                        >暂定下单</el-button
                      >
                      <el-button
                        type="primary"
                        v-if="currentNodeDetails?.hasSubmitPermission"
                ...
                <div class="auditLogs" v-if="currentNodeDetails?.auditRecords?.length">
                  <nodeAuditRecords
                    :list="currentNodeDetails?.auditRecords"
                    :taskName="selectedNode?.taskName"
                  />
                </div>
              </div>
            </div>
    ...
    >
        ...
        <el-form
          ...
          :rules="{
            tentativeOrderReason: [
              { required: true, message: '请输入暂定下单原因', trigger: 'blur' }
            ]
          }"
        >
          <el-form-item label="暂定下单原因" prop="tentativeOrderReason">
            ...
            <s-text-area
              v-model="currentNodeForm.tentativeOrderReason"
              :placeholder="`请输入暂定下单原因`"
              show-word-limit
              :maxlength="200"
              :rows="6"
            />
      ...
      <template #footer>
        ...
        <el-button type="primary" @click="submitOrderForm(false)" style="margin-right: 20px"
          >提 交</el-button
        >
      </template>
    </el-dialog>
  ...
  <!-- 节点进入 -->
  <component
    :is="showDialogContent"
    :info="currentTaskDetails"
    ref="showDialogContentRef"
    @success="getRefreshData"
  />
</template>
...
import { infoTables, documentList } from '@/views/OA/factory/plan/infoTablesOption'
...
const getRefreshData = async () => {
  ...
  try {
    ...
    newInfoTables.value = cloneDeep(infoTables)
    if (detailData.recoveryInfo && detailData.recoveryInfo.recycleUserId) {
      canOperate.value = false
      newInfoTables.value.unshift(
        // 回收信息
        {
          tableTitle: '回收信息',
          columns: 3,
