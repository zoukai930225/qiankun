<template>
  <el-dialog title="应用范围" v-model="dialogVisible" append-to-body :show-close="false" align-center width="600" style="padding: 0;">
    <template #header>
      <div class="header" style="padding: 16px 20px;border-bottom: solid 1px #F0F0F0;">
        <div>{{ currentDetail?.supplierName }}</div>
        <img src="@/assets/imgs/common/close.png" alt="" @click="dialogVisible = false" />
      </div>
    </template>
    <div class="container" style="padding: 4px 20px">
      <el-form :model="form" :label-width="tipInfo.width" :rules="rules" ref="formRef" :validate-on-rule-change="false">
        ...
        <el-form-item :label="tipInfo.titleValue" prop="attachmentUrl" v-if="!isTry">
          <transfer :upload-data="{ code: 'FS_SUPPLIER_PLAN' }" width="100%" v-model="form.attachmentUrl" :limit="5"
            :tip="'最多上传5张大小不超过20M的图片'" accept-type="all" />
        </el-form-item>
      </el-form>
    </div>
    <template #footer>
      <div style="padding: 10px 20px;border-top: solid 1px #f0f0f0;">
