<template>
  <scheme-dialog ref="detailsRef" name="nodeCategory">
    <template #card>
      <!-- <div> 详情内容区(外围卡片样式无需书写) </div> -->
      <el-form :model="dataValue" label-width="80" ref="formRef" :rules="formRules">
        <el-form-item prop="categoryCode" label="品类名称">
          <el-cascader
            style="width: 100%"
            clearable
            v-model="dataValue.categoryCode"
        ...
        <el-form-item label="基准时长" prop="basisDuration">
          <el-input-number
            v-model="dataValue.basisDuration"
            placeholder="请输入基准时长"
            :min="1"
          ></el-input-number>
        </el-form-item>
        <el-form-item label="时长单位" prop="durationUnit">
          <el-select v-model="dataValue.durationUnit" placeholder="请选择时长单位">
            <el-option
...
<script lang="tsx">
export default { name: 'nodeCategoryDetails' }
</script>
...
const formRules = ref({
  ...
  categoryCode: [{ required: true, message: '请选择品类名称', trigger: ['change', 'blur'] }],
  basisDuration: [{ required: true, message: '请输入基准时长', trigger: ['change', 'blur'] }],
  durationUnit: [{ required: true, message: '请选择时长单位', trigger: ['change', 'blur'] }]
})
...
const { onSearch, categoryTree, getCategoryTree, unitDic } = inject(
  `OaNewTempleteNodeCategoryDetailsTableForm`
) as FromProvideType & { categoryTree: Ref<CategoryItem[]>; getCategoryTree: () => void; unitDic: Ref<any>[] }
...
const changeCategory = (val) => {
  console.log(val)
  if (!val || val.length === 0) {
    dataValue.value.categoryCode = ''
    return
  }
  dataValue.value.categoryCode = val[val.length - 1]
}
...
const submit = debounce(async () => {
  ...
  request
    .post({
      url: dataValue.value?.id ? `/api/np/node/category/update` : `/api/np/node/category/save`,
      data: query
    })
    .then(() => {
...
provide('nodeCategoryDetails', { dataValue, bukValue, viewType, title })
