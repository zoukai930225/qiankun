<template>
  <el-popover placement="bottom-start" :width="255" :offset="0" trigger="hover" :popper-style="{
    borderRadius: '10px',
    padding: '14px 16px',
    height:
    ...
    <template #reference>
      <div class="popoverIcon">
        <img v-if="tab.status === '0'" src="@/assets/imgs/oa/node/node_unStart.png"
          style="width: 65px; height: 65px;margin-left: -4px;" />
        <div v-else style="
            display: flex;
            width: 56px;
            height: 56px;
          ...
          <div style="
            ...
            <div style="display: flex; flex-direction: column; align-items: center; height: 46px">
              <div style="
                  ...
                  line-height: 20px;
                  margin-top: auto;
                ">{{ getNodeWaitTime(tab.waitTime || '') }}</div>
              <div style="
                  font-weight: 500;
                  font-size: 12px;
                  color: #ffffff;
                  line-height: 14px;
                  margin-bottom: auto;
                ">{{ getNodeWaitTimeUnit(tab.waitTime || '') }}</div>
            </div>
          </div>
        </div>
      </div>
