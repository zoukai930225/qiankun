              ...
              >
                ...
                <el-table-column label="操作" width="160" fixed="right" align="center">
                  <template #default="{ row, $index }">
                    <el-button link type="primary" :icon="CircleCheck" @click="handleEdit(row)"
                      >编辑</el-button
                    >
...
// 新增
const handleAdd = () => {
  competiorAddDialogRef.value.open(null, currentId.value)
}
...
// 列表
const getList = async () => {
  const res = await getCompetorsList({
    ...params.value,
    npDesignPictureId: currentId.value
  })
...
const handleEdit = (row: any) => {
  competiorAddDialogRef.value.open(row, currentId.value)
}
// 删除
