    ...
    <el-dialog v-model="visible" :width="width" :before-close="handleClose" @open="resetForm"
      ...
      <template #footer>
        <div class="dialog-footer">
          ...
          <el-button type="primary" :loading="loading" @click="confirmDialog">提交</el-button>
        </div>
      </template>
    </el-dialog>
  </div>
...
import { designExamine } from '@/api/oa/package/designGallery/index'
...
const confirmDialog = async () => {
  ruleFormRef.value.validate(async (valid) => {
    if (valid) {
      try {
        const data = {
          designGalleryId: props.selectItem.designGalleryId, // 产品设计图库id
          examineStatus: Number(form.examineStatus), //审批状态（0:拒绝，1:同意）
          examineRemark: form.examineRemark //审核备注
        }
        loading.value = true
