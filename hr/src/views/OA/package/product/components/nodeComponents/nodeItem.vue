<template>
  ...
  >
    <div
      class="name"
      :style="{
        marginLeft: index === nodeList.length - 1 ? '-20px' : '-140px',
        opacity: tab.nodeCode === 'hideNode' ? 0 : 1
      }"
      >{{ tab.nodeName || '--' }}</div
    >
    <div class="nodeItem-head">
      <div
        class="step-line"
        :class="{
          'step-line-bottom':
      ...
      <div
        ...
        "
        :style="{
          marginLeft: bottomNodeFirstLineLeft(),
      ...
      <div
        ...
        "
        :style="{
          marginLeft: `${parallelBottomNodeCountByIndex(parallelHeaderNodeList, 3) * 366 - 170}px`,
          width: `${(parallelTopNodeCountByIndex(parallelHeaderNodeList, 2) - parallelBottomNodeCountByIndex(parallelHeaderNodeList, 3) - 1) * 366 - 54}px`,
          background:
            Number(
              getNodeByNodeCode(
                getPrallelEndNode(
      ...
      <div
        v-if="tab.nodeCode === 'hideNode'"
        style="position: absolute; width: 41px; height: 2px; opacity: 0.6; top: 25px; left: 44px"
        :style="{
          background:
...
const getStepLineBackground = (tab) => {
  ...
  if (tab && tab.isEndNode) {
    ...
    if (endNode && endNode.status == 'future') {
      return '#C2C2C2'
    } else {
      return '#0064ff'
    }
  } else {
    if (tab.status == 'future') {
      return '#C2C2C2'
    } else {
      return '#0064ff'
    }
