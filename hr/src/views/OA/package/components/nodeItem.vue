    ...
    <template #reference>
      <div>
        <div class="row-center topBg" :style="{ background: getTopBgColor(0) }">
          <div class="nodeName">{{ tab.taskName }}</div>
          <img class="timeIcon" src="@/assets/imgs/oa/node/node_time.png" alt="" />
          <div class="timeLabel">持续</div>
          <div
            class="timeValue"
            :style="{
              color: getNodeBgStatusColor(tab.duration || '')
            }"
          >
            <!-- {{ getNodeWaitTime(tab.waitTime) }}{{ getNodeWaitTimeUnit(tab.waitTime) }} -->
            {{ tab.duration }}
          </div>
        </div>
        ...
        <div class="bottomBg">
          ...
          <div
            class="nodeStatusLbl"
            :style="{
              color: getNodeBgStatusColor(tab.status || '')
            }"
...
import NodeDetail from '@/views/OA/package/product/components/nodeComponents/nodeDetail.vue'
