<template>
  <ContentWrap>
    ...
    <download-drawer biz-type="delivery_order" ref="downloadDrawerRef" />
  </ContentWrap>
</template>
...
defineOptions({ name: 'BiShipmentOrder' })
...
const getList = async () => {
  loading.value = true
  try {
    const data = await orderApi.getDeliveryList(dealParams(true))
...
const exportData = debounce(async () => {
  ...
  try {
    await orderApi.deliveryExport(dealParams())
