<template>
  <ContentWrap>
        ...
        <el-form ref="queryFormRef" :inline="true" :model="queryParams" label-width="66px">
          <el-form-item label="日期段" class="each-form upload-time">
            <el-date-picker v-model="times" type="daterange" start-placeholder="开始日期" format="YYYY-MM-DD"
              :clearable="false" value-format="YYYY-MM-DD" end-placeholder="结束日期" @change="handleDateChange">
          ...
          <el-form-item label="支付时间段" class="each-form upload-time times-select">
    ...
    <el-table :header-cell-style="appStore.headerCellStyle" v-loading="loading" :data="list"
      ...
      <el-table-column label="订单日期" prop="dataDate" min-width="120"
        :formatter="(row: any) => formatterValue(row, 'dataDate', '-')" />
      ...
      <el-table-column label="sku ID" prop="skuIds" min-width="280"
        :formatter="(row: any) => formatterValue(row, 'skuIds', '-')" />
      <el-table-column label="付款金额" prop="paymentAmount" min-width="120" align="right"
        :formatter="(row: any) => formatValue(row, 'paymentAmount')" />
      <el-table-column label="付款时间" prop="payTime" min-width="180"
        :formatter="(row: any) => formatTime(row.payTime, 'yyyy-MM-dd HH:mm:ss') || '-'" />
      <el-table-column label="订单创建时间" prop="createTime" min-width="180"
        :formatter="(row: any) => formatTime(row.createTime, 'yyyy-MM-dd HH:mm:ss') || '-'" />
      <el-table-column label="订单总额占比" prop="orderTotalRatio" min-width="120" align="right"
        :formatter="(row: any) => formatValue(row, 'orderTotalRatio')" />
      <el-table-column label="订单商品占比" prop="orderGoodsRatio" min-width="120" align="right"
        :formatter="(row: any) => formatValue(row, 'orderGoodsRatio')" />
      <el-table-column label="订单订单占比" prop="orderOrderRatio" min-width="120" align="right"
        :formatter="(row: any) => formatValue(row, 'orderOrderRatio')" />
      <el-table-column label="销售数量" prop="salesQuantity" min-width="120" align="right"
        :formatter="(row: any) => formatValue(row, 'salesQuantity')" />
      <el-table-column label="成本" prop="cost" min-width="120" align="right"
        :formatter="(row: any) => formatValue(row, 'cost')" />
      <el-table-column label="是否是商品卡" prop="isGoodsCardTxt" min-width="120"
        :formatter="(row: any) => formatterValue(row, 'isGoodsCardTxt', '-')" />
      <el-table-column label="是否是有效单" prop="isValidTxt" min-width="150"
        :formatter="(row: any) => formatterValue(row, 'isValidTxt', '-')" />
      <el-table-column label="无效单佣金" prop="wxdCommission" min-width="120" align="right"
        :formatter="(row: any) => formatValue(row, 'wxdCommission')" />
      <el-table-column label="货品属性" prop="productAttributes" min-width="150"
        :formatter="(row: any) => formatterValue(row, 'productAttributes', '-')" />
      <el-table-column label="订单状态" prop="orderStatus" min-width="150"
        :formatter="(row: any) => formatterValue(row, 'orderStatus', '-')" />
      <el-table-column label="商品价格" prop="productPrice" min-width="120" align="right"
        :formatter="(row: any) => formatValue(row, 'productPrice')" />
    ...
    <download-drawer biz-type="pay_order" ref="downloadDrawerRef" />
  </ContentWrap>
</template>
...
const getList = async () => {
  loading.value = true
  try {
    const data = await orderApi.getPayOrderList(dealParams(true))
...
const exportData = debounce(async () => {
  ...
  try {
    await orderApi.payOrderExport(dealParams())
