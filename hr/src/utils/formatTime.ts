 ...
 * 获取昨天开始时间、截止时间
 */
export function getLastDay(): [dayjs.ConfigType, dayjs.ConfigType] {
 ...
 * 获取最近N天的开始时间、截止时间
 */
export function getLastNDays(n: number): [dayjs.ConfigType, dayjs.ConfigType] {
  const lastWeekDay = dayjs().subtract(n, 'd')
  const yesterday = dayjs().subtract(1, 'd')
  return getDateRange(lastWeekDay, yesterday)
}
 ...
 * @returns 结束日期
 */
export function getDateAfterNDays(date: string | Date, n: number) {
